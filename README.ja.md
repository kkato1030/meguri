...
**GitHub pr-reviewer** は任意の外部レビューループ（`runs.loop_kind = "pr-reviewer"`）で、project × kind で切り替えます（`review.guard.plan` — 既定 ON、旧 spec reviewer / `review.guard.impl` — 既定 OFF）。開いた PR を独立した `pr-reviewer` profile でレビューし、同じ形式で verdict を残します — `meguri/pr-review` commit status + PR 本文の折り畳み `<details>` — **inline スレッドは決して作りません**。したがって **fixer** は反応せず、AI↔AI の ping-pong は畳まれたままです。**impl** のレビューは品質ゲートではなく**安全 tripwire** です（[ADR 0025](docs/adr/0025-guard-is-a-safety-tripwire-advisory-does-not-block.md)）: 品質は self-review が収束済みなので、verdict は `clean | advisory | blocking` の三値で、止めるのは閉じた安全カテゴリ（`security` / `data-loss` / `cost` / `performance`）を名指しした `blocking` のときだけ（赤 status → `needs-human`）。カテゴリを名指しできない・疑わしいだけのものは `advisory` で、PR 本文に記録されますが status は緑のまま — auto-merge は止まりません。**plan** のレビューは従来どおりの品質ゲートで（findings は spec_fixer が収束させます）、spec ラベルも駆動します（clean → `spec-ready`）。人間にとって赤い pr-review チェックは **advisory** です（`meguri/pr-review` を required check に指定しない限りマージは止めません）。auto-merge にとっては **gate** です（後述）。
...
**fixer 家族**（`fixer` / `ci-fixer` / `conflict-resolver`）はもうループごとに自己 discover しません。トリガは **1 つの level-triggered reconciler の arm**（ADR 0012 スライス3）になりました: 単一の `next_step` が全 open PR を一括観測し、各状態をちょうど 1 つの arm が所有します — Conflict → conflict-resolver、required CI 赤 → ci-fixer、meguri 待ちの未解決スレッド → fixer、そして merge tail（arm / update-branch / stuck）。だから状態がループの隙間に落ちることも二重所有も起きません(property test で保証)。dispatch は **workqueue + resync**: reconciler が arm ごとに `queued` run を作り、マージ近接の優先度順に配り、症状が残ったラウンドの後は指数バックオフで間隔を空け、fixer 家族は家族横断の DB インデックスで PR ごと同時 1 本に制限します。その claim は instance 名入りのマーカーコメント(`<!-- meguri:claim … -->`、meguri 自身が書いたものだけ信頼)として PR に射影され、`meguri:working` は人間向けの表示ラベルへ格下げされました(ADR 0026 / 0027)。
...
# webhook_url = "https://hooks.slack.com/services/..."  # 選んだイベントを webhook へ push。${ENV} 展開可。省略で無効
# kind = "slack"       # slack | ntfy | json。省略時は URL のホストから自動判別
# events = ["awaiting_human", "escalation", "schedule.failed", "schedule.skipped"]  # allowlist。既定は ["awaiting_human"]
throttle_secs = 60     # 同一キーの連続通知の最短間隔(秒)
# [[projects]]
#   notify = { labels = ["human:todo"] }  # meguri が該当ラベルの issue を起票した時にも通知
