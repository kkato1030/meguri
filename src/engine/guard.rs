...
/// The guard's folded `<details>` block within a PR body, if present:
/// everything from [`GUARD_BODY_MARKER`] to the end (the guard block is always
/// last — see [`upsert_guard_details`]). `spec_fixer` reads it to feed the
/// plan findings back to the fixing agent (issue #188); returns `None` when the
/// body carries no guard block.
pub fn extract_guard_details(body: &str) -> Option<String> {
    let idx = body.find(GUARD_BODY_MARKER)?;
...
mod tests {
    ...
    #[test]
    fn extract_guard_details_pulls_the_folded_block() {
        // No block: nothing to feed a fixer.
        assert_eq!(extract_guard_details("Refs #5.\n\nBody text."), None);

        // With a block: everything from the marker to the end (issue #188).
        let body = upsert_guard_details(
            "Refs #5.",
            &guard_details(&cp(Kind::Plan, "aaa"), ReviewVerdict::Findings),
        );
        let block = extract_guard_details(&body).unwrap();
        assert!(block.starts_with(GUARD_BODY_MARKER));
        assert!(block.contains("- missing acceptance criteria"));
        assert!(!block.contains("Refs #5."), "only the guard block: {block}");
    }

    /// The plan guard's settle drives the spec label state machine (ADR 0008
    /// §3): clean flips `spec-reviewing → spec-ready`, findings keep
    /// `spec-reviewing` so the next push (spec_fixer, issue #188) re-guards.
    #[tokio::test]
    async fn plan_settle_drives_the_spec_label_state_machine() {
        ...
        assert_eq!(
            forge.commit_status_of("deadbeef", GUARD_STATUS),
