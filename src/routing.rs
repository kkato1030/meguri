...
pub const KNOWN_ROLES: &[&str] = &["worker", "fixer"];
...
pub fn routing_role_for_loop(loop_kind: &str) -> &'static str {
    match loop_kind {
        ...
        // "worker" and anything unrecognized.
        _ => "worker",
    }
}
...
pub fn recommended_chain(role: &str) -> &'static [&'static str] {
    match role {
        // The bulk of consumption (worker) and the narrow-scope fix-up work
        // (fixer, incl. ci-fixer / conflict resolution) both land on Sonnet —
        // close to Opus on coding at roughly half the quota/price.
...
"#,
    ...
    fn deprecated_role_keys_steer_the_renamed_roles() {
        // Configs still using pre-#167 keys resolve to the current roles:
        // `spec-worker` → `worker`, `conflict-resolver` / `ci-fixer` →
        // `fixer`. validate() accepts them all.
        let cfg = cfg_from(
            r#"
[routing]
mode = "manual"
...
"#,
        ...
        // The old `conflict-resolver` key also still steers its new role.
        let cfg2 = cfg_from(
            r#"
[routing]
mode = "manual"
