...
**Internal self-review** is an *internal phase* (ADR 0006): the author reviews its own work before the PR is ever pushed, so the review→fix ping-pong never touches GitHub. Between `validate` and `open-pr`, round 1 fans out to every configured `[[review.reviewers]]` entry (optionally on heterogeneous models/profiles — ADR 0023; empty is the historical single reviewer, unchanged) and each writes `{verdict, findings[]}` applying its lenses (`review.lenses`, default `correctness / tests / simplicity / security`); findings are merged by mechanical union, no orchestrator agent involved. Every finding is `kind: defect` or `kind: decision` (no severity — findings are all blocking by definition) and lands on a cumulative **ledger** that survives across rounds and crash resume. A **fix turn** addresses open ledger entries (fixed or waived, with a reason) and commits, the project check re-runs, and round 2+ re-reviews under a single anchor model that checks *resolution of the existing ledger plus new-only findings*, not a from-scratch re-review (ADR 0022). Escalation to `needs_human` is decided by behavior, not round count: a finding still open after two fix attempts (ping-pong), a disputed recorded decision, or a round-1 reviewer's `needs_human` verdict — the latter is never taken at face value: any flag is confirmed by one anchor-model turn before it escalates, so a lone reviewer's false alarm doesn't multiply with the reviewer count (ADR 0023 §2). Reaching `review.max_rounds` with only ordinary open findings left instead runs one final fix + validate and publishes, noting in the PR body that the last fix wasn't re-reviewed (the pr-reviewer / human merge gate is the backstop). Nothing is posted to the conversation — review turns run under the `self-reviewer` routing role (so they can be a different model than the author), and the outcome is recorded off the conversation timeline: a `meguri/self-review` commit status on the pushed head and a folded `<details>` in the PR body. Set `review.enabled = false` to skip it (e.g. an external bot covers reviews).

**GitHub pr-reviewer** is the optional external review loop (`runs.loop_kind = "pr-reviewer"`), toggled per project × kind (`review.guard.plan` — on by default, the old spec reviewer — and `review.guard.impl` — off by default). It reviews the opened PR under an independent `pr-reviewer` routing role and records its verdict the same way — a `meguri/pr-review` commit status + a folded PR-body `<details>` — **never inline threads**, so the **fixer** never reacts to it and the AI↔AI ping-pong stays retired. The **impl** review is a *safety tripwire*, not a second quality gate ([ADR 0025](docs/adr/0025-guard-is-a-safety-tripwire-advisory-does-not-block.md)): self-review already converged quality, so its verdict is `clean | advisory | blocking`, and it only stops the merge on a `blocking` verdict that names a closed safety category — `security` / `data-loss` / `cost` / `performance` (a red status that escalates to `needs-human`). Anything it cannot pin to a category, or merely suspects, is `advisory`: recorded in the PR body but leaving the status green so auto-merge proceeds. The **plan** review is unchanged — it stays a quality gate (spec_fixer converges its findings) and drives the spec labels (clean → `spec-ready`). For a human, a red pr-review check is *advisory* (it does not block the merge unless you make `meguri/pr-review` a required check); for auto-merge it is a *gate* (below).
...
# events = ["awaiting_human", "escalation", "schedule.failed", "schedule.skipped", "sweep.degraded"]  # allowlist; default ["awaiting_human"]
...
max_rounds = 3    # max self-review rounds per run; past the cap, behavior branches (ADR 0022) —
                  # ping-pong / needs_human / a disputed decision escalate, otherwise a final fix + publish
...
# [[review.reviewers]]        # optional: fan round 1 out to parallel reviewers (ADR 0023). Omit for
# profile = "claude-opus"     # the historical single self-reviewer. Host-only ([review] lives in host
# lenses = ["correctness"]    # config.toml, never a repo meguri.toml — profile is a trust declaration).
