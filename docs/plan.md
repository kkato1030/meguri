...
本計画はゼロからの新規開発である。ただし旧 meguri(v1: issue 駆動 autonomous loop / v2: 増分書き直し)で実費を払って得た教訓は、コードではなく**設計原則として持ち込む**(§3.5)。旧実装の履歴と ADR 群(失敗カタログ)はオーナーのローカルクローンの archive ブランチに保存されており、各増分の設計前に該当 ADR を参照する。
...
* **失敗カタログ参照。** 旧リポジトリの `docs/adr/`(ローカル archive ブランチ)は失敗カタログ。各増分の設計前に該当 ADR を読む。解を写す義務はないが、記録された失敗は再発させない。
