...
- **schedule discovery は read の前に `origin/<default_branch>` を fetch し、失敗時は repo 層を abstain する**
  ...
  見る。fetch には時間上限を掛け、hang しても host schedule の発火を止めない。
- **schedule 定義と `body_file` は同一 commit（同一 SHA）から読む**。resolver は fetch 後に default branch tip を
  1回 SHA に固定し、`meguri.toml` も fire 時の `body_file` もその SHA で読む。並行 fetch で ref が動いても、
  定義は commit A・本文は commit B、という割れ（B で body_file が消えていれば発火喪失）を避けるためである。
...
- **repo config の parse は「全 repo-eligible key を知る envelope」1つで行い、完了契約の pin 型は据え置く**。
  schedules の存在で `deny_unknown_fields` が壊れないよう envelope が schedules を寛容に受けつつ、host-only key
  混入は引き続き検出する。pin 用 `RepoConfig`（`Checkpoint` に serialize される型）は envelope から派生する
  **既存のまま**の型で、`schedules` フィールドを持たない。これにより保存済み checkpoint の JSON 形が変わらず、
  rollback で旧バイナリが新 checkpoint を読める（下記）。
- **rollback には順序が要る**: default branch の `meguri.toml` に `[[schedules]]` が残ったまま #222 前の
  コードへ revert すると、旧 `RepoConfig`（`schedules` を知らない）が worktree の `meguri.toml` を
  `deny_unknown_fields` で弾き、run の pin が host-only に落ちて `check_command` 等を失う。**先に default branch
  から `[[schedules]]` を除去**してからコードを revert する。pin 型を据え置いたので**保存済み checkpoint は壊れ
  ない**（旧 decoder が読める）が、worktree ファイルの互換は過去のコードを変えられない以上この順序でしか
  担保できない（詳細は spec の migration）。
