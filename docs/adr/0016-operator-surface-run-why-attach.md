...
`run` は `--issue N`、`attach` は位置引数(issue 番号か run id)という既存文法に揃え、`why` は
`meguri why <id> [--pr N] [--project P]`(`<id>` は issue 番号か run id、`--pr` で PR、`--project`
で複数 project の曖昧さ解消、local mode では local task id)とする。`why` は **fresh observation**
(informer cache ではなく1回きりの observe)を回して Step + 理由を出す **読み取り専用**コマンドで、
forge 書き込みも run 生成もしない(CLI テストで担保)。詳細な例と検証は spec の決定9に置く。
