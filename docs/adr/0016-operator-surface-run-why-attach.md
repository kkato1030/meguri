...
3 動詞が受ける **identity 集合**を閉じて定める: issue 番号 / PR 番号 / run id / local task id
(repo / project は含めない)。`why` は型付きの相互排他フラグ(`--issue` / `--pr` / `--run` /
`--task` のちょうど1つ)で identity を一意に指す — 位置引数だと issue 番号・PR 番号・run id が
同値のとき曖昧になるため。`why` は **fresh observation**(informer cache ではなく1回きりの
observe)を回して Step + 理由を出す **読み取り専用**コマンドで、forge 書き込みも run 生成も
しない(CLI テストで担保)。`run` は役割を指定せず、その identity を観測して reconciler が選んだ
arm を dispatch する(worker 固定ではない)。具体的な CLI 文法・例・実在 id 形式・検証は spec の
決定9に置く(spec は使い捨てなので、恒久的な「3 動詞・4 identity・役割非指定」の決定のみここに残す)。
