...
```bash
apm install -g kkato1030/meguri/skills/meguri#<tag>
```

`-g` でユーザースコープに入り、Claude Code のユーザーレベルスキルとして展開される。これで
meguri 未導入のリポジトリでもスキルが発火する。
...
- ユーザーは `apm install -g kkato1030/meguri/skills/meguri#<tag>` の 1 コマンドで獲得スキルを
