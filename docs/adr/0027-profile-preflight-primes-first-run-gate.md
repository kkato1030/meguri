...
- **実行**: prime は `tokio::process` で async に走らせ、timeout・reap も `.await` する。同期実行だと
  ...
  （issue #235 self-review f4）。
- **「一度だけ」の統制**: 「済み」状態は **gate identity** 単位（`(command, config_dir, argv)` —
  `gate.rs` の `GateTarget` と同じ dedup キー）のマーカーで持ち、プロセス内 async ロックで確認→prime→
  記録を直列化し、成否を記録して二度は試みない（claim-once）。単一の worktree フラグだと非 yolo profile
  が yolo profile の必要な prime を握り潰し、非直列だと並列 reviewer が prime を重複実行し、成功時のみ
  記録だと失敗後に毎 spawn 再 prime する — その3つを塞ぐため（issue #235 self-review f6/f7/f8）。
- **prime の対象 pane は2か所**: `spawn_agent_pane`（worker/planner/fixer/pr-reviewer）と
  `spawn_advisor_pane`（collab advisor）。後者も fresh dir で同じ対話 CLI を直接起動しゲートに当たる
  （issue #235 self-review f9）。
- **prompt injection 面（f10）**: prime は yolo posture で worktree 上の実モデルターンを走らせるため、
  repo の CLAUDE.md や外部 PR 内容の injection が no-op を上書きしツール操作を誘発しうる。緩和として
  既定 prime argv にツール封じを足す（bypass 受諾はモード、folder trust は cwd に紐づきツール実行を
  要さないので receipt は書ける）。残余は明示的に受容する — 直後の実 pane ターンが同じ worktree・同じ
  CLAUDE.md 上で full-yolo で走る以上、worktree を信頼して yolo で動かす判断は ADR 0012 で既に下され、
  外部内容が injection 面であることは ADR 0024 で既に明示済み。prime はこの境界を新設せず時刻を早めるだけ
  で、ツール封じでその前倒し分も無効化する。ADR 0024 の「面を明示し既存の緩衝を防御と位置づける／実行時
  sanitize は入れない」方針に揃える。ツール封じ prime が両 receipt を書くことは実機で検証する。
...
- 新規 worktree の gate identity ごとに一度だけ軽い prime が走る。フォルダ信頼はパス単位なので
  prime は必ず worktree の cwd（advisor は advisor_dir）で走らせる。
- pre-flight が書かせる状態は「人が一度受諾したのと同じ」もの。yolo profile では bypass 受諾が
  config-dir に残る（yolo pane と同じ状態）／非 yolo profile では書かれない。meguri をロールバック
  してもその状態は `~/.claude.json` に残るが、CLI 側の資産であり meguri の管理外。
