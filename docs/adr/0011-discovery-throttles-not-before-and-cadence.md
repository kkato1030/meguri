...
現状の回避策は「人間または外部 cron が小出しにトリガーラベルを貼る」で、キューの一括編成(週次でまとめて起票し、あとは meguri に任せる)ができない。週次でキューを issue として一括編成する運用にすると、日次ペース必須のタスク(SNS 投稿)だけがキュー駆動に載せられず外部 cron に残る、という具体的なつまずきがこの決定の動機である。#146(1/2, ADR 0009)で「起票の入口」を時刻駆動化したのと対になる形で、「消化の調速」を discovery に足す。この2つで、外部 cron なしの時刻駆動運用が入口(起票)と出口(消化)の両輪で完成する。
...
- **cadence rule に一致するラベルは issue あたり最大1つ(v1)。** issue のラベルが 2 つ以上の rule に一致する場合は fail-closed(discover しない、手動 `meguri run` はエラー)とし、CLI が理由を見せる。
...
- **cadence バケツは run 作成の `INSERT` と同一文で確定させ、以後不変。** discover が「この issue はラベル `L` の cadence 対象」と判定した事実を、run 行を作る `INSERT` 自体に含めて刻む。後追いの `UPDATE` による2手順刻印は採らない — run 作成と刻印の間でプロセスが落ちると「run は存在するのに `cadence_label` が NULL」の行が残り、窓内 COUNT から漏れて次 tick が同バケツをもう1件流す(上限超過)。刻印を `INSERT` に含めればこの crash window は構造的に存在しない。後から issue のラベルが変わっても、過去の消化実績は動かない。
- **run が属せる cadence バケツは1つ(`runs.cadence_label` は単一)。よって v1 は「cadence rule に一致するラベルは issue あたり最大1つ」を要求し、複数一致は fail-closed とする。** GitHub issue は複数ラベルを普通に持てるため、例えば `sns` と `newsletter` の両 rule に一致する issue の run をどちらか一方だけに刻むと、刻まなかった側の窓内 COUNT から漏れ、その上限が守られない。両方に数える多重刻印(join table 化)は「消化 = 窓内の run の COUNT」という単純な不変条件を複雑にする割に、運用上の実例がまだ無い。v1 は複数一致を discovery のサイレントスキップ(+ CLI での理由表示)で明示的に締め出し、手動 run もエラーにする — どのバケツで数えるかを実装が勝手に選ばない。多重刻印が本当に要る運用が現れたら別 issue で再考する。
...
- **手動 run(`meguri run --issue`)は cadence ゲートをバイパスするが、消化には数える。** 手動実行は discover / scheduler を通らず `store.create_run` を直接叩く人間の明示的な上書きなので、残枠チェックで止めない(常に走らせる)。ただしその issue が cadence 対象ラベルを持つなら run 作成の `INSERT` で `cadence_label` を刻む — 「run 作成が消化の唯一の証跡」(根拠3)を手動経路でも守るためで、これを刻まないと同日 `watch` が同ラベルの別 issue を追加消化して上限を超える(手動実行が消化枠に見えなくなる)。ラベルが cadence rule に一致しなければ従来どおり `cadence_label` は NULL。複数 rule に一致する場合だけは例外的にバイパスさせず、エラーで止める(上記のとおり、どのバケツで数えるかを実装が選べないため)。
...
- discover ゲートの順序は `hold/working` → 未 ship → **not-before** → dependencies → **cadence** と同じ層に並ぶ(claim より前)。**cadence を最後に置く**のは、共有残枠(`max - consumed`)を消費してよいのは他の全ゲート、特に dependencies を通過した actionable な候補だけだからだ。cadence を dependencies より前に置くと、blocked な古い issue が残枠を食ってから dependencies で落ち、後続の actionable な同ラベル issue が残枠ゼロで毎 tick スキップされ続ける(サイレントスキップの収束性が壊れる)。not-before は issue 単位のゲートで共有残枠を持たないため、dependencies との前後関係は結果に影響しない。
- `runs` に `cadence_label`、`tasks` に `not_before` カラムが増える(migration 追加)。`Target` に cadence バケツを載せ、scheduler が run 作成の `INSERT` に含めて刻む(後追い `UPDATE` はしない)。
