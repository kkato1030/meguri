...
**dashboard 強調は小改修を要する**(本 ADR の唯一の実測リスク検証結果)。`meguri top` の `top_refresh` は `store.list_runs(true)`(= `queued`/`running`/`interrupted` のみ)を読むため、`Succeeded` で park した run は行に現れず、`▶` 強調も出ない。`list_runs(true)` は scheduler も駆動対象の抽出に使う共有関数なので**意味を変えてはならない**。よって review park だけを拾う専用クエリを追加し、dashboard 側だけがそれを強調行として合流させる(pane が無くても表示する — 動作対象は pane ではなく PR)。ここで条件を「非 active かつ `AwaitingHuman`」で済ませてはならない: `AwaitingHuman` は turn 側でも立ち、`update_run_status` はそれを消さない。だから人間待ちのまま stop/cancel された run や、failed/skipped で終わった run まで釣れてしまう。拾うのは**正常終了した review park だけ** — `status='succeeded'` かつ pr-reviewer の run に限る。
...
- park シグナルのヘルパは kind 非依存に作る。`pr_reviewer.rs` の settle が findings/clean 両分岐から同一ヘルパを呼び(clean は `plan_delivery=separate` のときだけ)、#188 の spec_fixer がラウンド上限超過エスカレーションで再利用できる。
