...
- 関連: ADR 0008(`0008-symmetric-plan-impl-review-loop.md`、#132、main 着地済み — **番号が `0008-agent-instructions-via-apm.md` と重複しているため、本文書の「ADR 0008」は常に前者・対称化 ADR を指す**)、ADR 0007(merge-watch は fixer 系に委譲)、ADR 0005(ラベル二軸)、#188(findings park の自動修正ループ — 本 ADR は能動シグナル側)
...
一方 spec レビュー(guard(Plan)、`src/engine/guard.rs` — ADR 0008 の対称化で旧 `spec_reviewer` を置換済み)には、**run が正常終了した後**に人間の判断を待つ park が存在する:

- **findings park** — guard(Plan) が findings を返すと `settle` は `meguri/guard-review` の commit status(failure)を書き、レビューを PR 本文の `<details>` に折り込んで `meguri:spec-reviewing` を維持したまま run を `Succeeded` で終える(`src/engine/guard.rs:661-705`)。同じ head は二度レビューされない(commit status が dedup key、同 `:205-213`)ため、次の push があるまで PR は静かに固まる。
- **clean park** — `plan_delivery=separate`(既定、`src/config.rs:717-722`)では clean で `spec-reviewing → spec-ready` に遷移した後、spec PR のマージは人間の仕事になる(`spec_worker` は combined でしか discover しない — `src/engine/spec_worker.rs:66`)。combined では `spec_worker` が自動継続するため park ではない。
...
なお findings park を「まず自動で直す」ループの欠落は #188(spec_fixer)として別途扱う。役割分担は #188 = 自動修正側、本 ADR/#153 = 能動シグナル側。#188 着地までは findings park も即座に人間の仕事なので本シグナルの対象であり、着地後は findings 側の page をラウンド上限超過エスカレーション時へ移す(#188 側で調整)。本 ADR が守る不変条件は「**無標識の park を作らない**」ことである。

## Decision

guard(Plan) が park する終端で、既存の `InteractionState` + `Notifier` レールを **ワークフロー階層の待ちにも流用**する。具体的には park 時に:
...
- **(B) `meguri:needs-human` ラベルを貼る** — ADR 0008 の needs-human 昇格は auto-merge guard(Impl) **failure** に限定されている。findings は失敗ではなく**正常な人間ゲート**であり、ball 軸(ADR 0005)の needs-human とは意味が違う。加えて `needs-human` はループの締め出し(ADR 0007 のデッドロック注意)を招きうる — #188 の spec_fixer は `needs-human` の付いた PR を discover しないので、自動修正の芽を摘んでしまう。却下。
...
- park シグナルのヘルパは kind 非依存に作る。`guard.rs` の settle が findings/clean 両分岐から同一ヘルパを呼び(clean は `plan_delivery=separate` のときだけ)、#188 の spec_fixer がラウンド上限超過エスカレーションで再利用できる。
- #188 着地後、findings 直後の page は上限超過時へ移る(調整は #188 側)。本 ADR のシグナル機構自体は変わらない。
- loops.md の ADR 索引(§5)更新は、#132 着地に伴う doc 追随(同 doc §6 に予告済み、未了)に合流させる — 本 ADR 単独では触らない。
