...
| repo 可(host が上書き可) | `check_command`、`language`、`pr.draft`(将来: `clean`※、`prompts` #149、`worktree_setup` #139) | プロジェクト内在。どのホストで回しても同じであるべき事実 |
| repo 可・要判断 | `schedules`(#146) | 反映には write 権限が要り境界内だが、常設の実行トリガを repo に置く段差があるため初期は host 側に据え置く(論点は issue #146 側) |

※ `clean` は境界原理上は repo 可(プロジェクト内在)だが、**初期実装スコープから外す**。cleaner loop は
通常の run flow に乗らず、`interval_hours` を run がまだ存在しない discover 時に、`ignore` を独自
checkpoint の sweep 内で読むため、本 ADR の claim 時 pin 機構が届かない。repo 化には cleaner 専用の
読み込み・pin・precedence の設計が要る(将来 issue)。
...
1. **claim 時 pin。** 完了契約を含む repo-eligible 値は、run claim 時(初回 worktree 準備時)に
   ...
   検証・PR 作成・prompt はこの pin 値だけを読む。**pin の persist は最初の agent turn が走るより
   必ず先**に置く — worktree 準備〜pin の間に crash しても、その区間で agent は一度も走っていない
   から、resume 時の再読が改竄に晒されることはない(具体配置は spec)。
