...
**issue 投入は capture-first にする。投入(capture)は決して LLM を経由せず、待たせず、一度成功した capture を後段の失敗に取り消させない。AI による整形(refine)はその後追いの best-effort であり、issue↔pane↔session の寿命モデル(#92)の外にある一発の headless 処理として扱う。** これを 4 つの不変条件に固定する。

1. **capture は LLM を経由せず、成功した capture は取り消されない。** issue 作成は `Forge::create_issue` 直で、番号と URL を即座に返す。`create_issue` が成功した後は、refine が失敗しても・タイムアウトしても・Ctrl-C されても、issue は raw のまま GitHub に残り、コマンドは capture 成功として報告する(silent に issue が消えることは決してない)。「失敗させない」の射程はここまで — `create_issue` 自体は認証切れ・ネットワーク障害・設定不備で失敗し得るし、そのときは issue が存在しないのだから、通常のエラーとして報告する。capture-first が排除するのは「LLM や refine の不調が投入を巻き込む」ことであって、forge 障害の隠蔽ではない。
