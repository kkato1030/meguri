...
- ただし失敗経路でも **`working` claim は解放する**。touchability 判定は `working` を
  `needs-human` より先に見るため、claim が残ると「次 sweep に委ねる」が成立しない
  （PR も issue も再発見されない）。claim を外してこそ再試行できる。
...
informer キャッシュではなく forge の live read（`pr_comments_meta`）で行う — これがキャッシュ
遅延に勝つ肝である。marker は **自著（`viewerDidAuthor`）のコメントに限って信頼する**:
第三者が公開 head/reason の marker を先に投稿して正当なエスカレーションを握りつぶす（event と
human paging まで抑止する）攻撃を防ぐため。ADR 0027 が claim marker に課したのと同じ自著判定を、
そのまま escalation marker にも適用する。
...
- forge write の順序が「label → (dedup 判定) → working 解除 → comment」に固定される。
  ...
  `working` 解除は成立・defer・dedup のどの経路でも行い、claim を残して次 sweep を止める事故を
  防ぐ。
- dedup を自著コメントに限定したことで、第三者が marker を偽造してエスカレーションを封じる
  経路を塞ぐ（ADR 0027 と同じ no-forge 前提を escalation にも広げた）。
