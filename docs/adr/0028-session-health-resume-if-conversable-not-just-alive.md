...
これでシェルへ nudge を打ち込む事故と、無意味な nudge 待ちが消える。

ただし session id を消すだけでは足りない: pane 行の `mux_pane_id` が残ると、次の dispatch が
その生きた(素のシェルの)pane を adopt して trigger を打ち込んでしまう。よってローテート
経路と `Absent → PaneDied` 経路では pane を明示的に kill + reclaim し、加えて adopt 分岐に
「live pane が `Absent` なら adopt しない」ゲートを置く。これで再 dispatch は必ず resume か
fresh spawn へ進み、素のシェル落ちループを断てる。
...
### 4. 診断に pane 末尾 N 行を同梱する(サニタイズ必須)
...
ただし pane tail は agent が吐いた任意の文字列であり、そのまま Forge コメントや通知へ出すと
端末エスケープ・任意 Markdown・メンション・秘密の露出面になる。よって**出す先で扱いを分ける**:
生の tail はローカルイベント(sqlite)限定とし、Forge/通知へ出す時は必ずサニタイズする —
ANSI・制御文字を除去し、バイト上限で切り詰め、コードフェンスで囲んで Markdown を中和する。
秘密の網羅 redaction は原理的に困難なので範囲外とし、注入と露出量を抑える方針を採る。
