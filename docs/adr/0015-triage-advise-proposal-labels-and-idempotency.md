...
2. **再トリアージは内容ハッシュで判定する。** 提案ラベルだけを持つ issue は、`(title, body)` の
   SHA-256(reconcile ループの `tasks::body_digest` を流用、issue #142)が前回の提案時と変わっていれば
   候補に戻る。ハッシュは issue の updatedAt より頑丈(ラベル操作だけの更新に反応しない)で、
   ローカル状態も要らない。
