...
- `LedgerEntry` に `anchor_verified: Option<bool>` を、`Finding` に `quote: Option<String>` を足す。
  ...
  `Some(false)`(差し戻し後もなお照合失敗の**再リスト**)。`anchor_verification` 無効時は全 entry が `None` で
  **serialize されない**ので checkpoint は byte-for-byte 不変、旧 checkpoint も serde default で読める。
- **`anchor_verified` は「監査/表示専用」ではなく、`status` と直交する制御フラグである(f14)。** 収束軸は
  従来どおり `status`(open/fixed/waived)で、収束 = open な entry 数がゼロ。`anchor_verified` は status を
  動かさない。ただし `Some(false)` は entry を **非 actionable** にする制御状態として使う: (a) fixer の
  対象集合から除く、(b) fix file の「open には disposition 必須」検証の母集団から除く。open のままなので
  収束はせず、その phase は cap → final-fix publish に落ちる(§2)。つまり `anchor_verified` は
  「status(収束)」「actionability(fixer が触るか)」「stats/表示」の3つに効く — 単なる監査フラグではない、
  と明記して f14 の矛盾を解く。境界: **open/fixed の遷移は `status` だけが持ち、`anchor_verified` は
  actionability と表示を制御する**。
