...
冪等に実体化する(既にあれば no-op)。

clone 失敗(認証・ネットワーク・slug 誤記)は silent skip しない。ただしこの失敗は run/issue/PR
を1つも持たない tick 最上段で起きるため、**座標対象に付く needs-human ラベルや run 前提の
通知には載せない**。性質は「run が人の判断を待つ」ではなく **operator の config/認証/ネット
ワークの問題**であり、level-triggered に毎 tick 再試行して operator が直せば自己修復する。
よって loud の宛先は **event(`repo.clone.failed`)+ tick ログ + doctor 表示**とし、doctor を
「設定は健全か」を見る唯一の人間向け表示面に据える。project-level の新しい escalation/通知面は
作らない(座標モデルを広げる別判断。必要なら別 ADR)。
