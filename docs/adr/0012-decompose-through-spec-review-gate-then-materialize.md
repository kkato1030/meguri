...
- **子の識別を forge 側で自己完結させる**。子1件を安定 key(`提案の親 + index`)を body に入れて
  作成する。key は `create_issue` に渡す body の一部なので「作成」と「発見可能」が単一 API 呼び出しで
  同時に起きる。作成前に必ず key で既存子を検索し、あれば採用・無ければ作成する(adopt-or-create)。
  親 body の台帳は検索の高速パスにすぎず、正しさの根拠は子 body の key。
...
- 材料化は取り返しのつかない forge 書き込みを含むため、kill-switch(config)・子 body の安定 key・
  ...
  さらに分解できない。
- `Forge` トレイトに 2 メソッドを足す:PR を未マージで畳む `close_pr`(commit point)と、body の
  key で既存子を照合する `find_issue_by_marker`(adopt-or-create)。現トレイトに PR close 手段が
  無いため、これが無いと materialization は完了できない。
