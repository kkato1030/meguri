...
3. **agent の在否を mux の一級の能力にする**。pane 生存とは別に「その pane に agent がいるか」を
   ...
   同じ有界の機械に載せる。nudge は「在否確認と送信を分離しない」原子的操作にする —— 確認と
   送信を別呼び出しにすると、その隙に agent が exit してシェルへ打ち込む TOCTOU が残るため。

4. **エスカレーションに診断を同梱するが、裁定には使わない。しかも外へ出す前に sanitize する**。
   agent_quiet で needs-human に上げるときは pane 末尾 N 行(既定 25)をコメントとイベントに添える。
   これは人間が真因を掴むための read であって、成否の裁定には一切使わない —— overview の
   「画面を読んで成否判定しない」原則、ADR 0025・0026 の立て付けをそのまま踏襲する。加えて、
   pane には credential・PII・任意 Markdown が映りうるので、外部(Forge コメント)へ出す前に
   制御文字除去・秘密のレダクション・バイト上限・fence 脱出防止を必ず通す。
