...
このリポジトリは OSS 公開連作(issue #113〜#119)により public 化を予定している。
public 化は (4/7) #116(公開作業チェックリスト)で実施する。本ドキュメントは
**public 化後の GitHub リポジトリ設定を目標状態として** 記述し、#116 が実行する
...
**#116 が実行されるまでは、このリポジトリは実際には private + Free のままであり、
automerge のゲートは引き続き ADR 0004 本文どおり「チェック緑を待つ Renovate 側
マージ」で機能している。** 現行(移行前)の設定・検証コマンドは末尾の
[「public 化前(現行)の設定」](#public-化前現行の設定) にまとめて残す。
...
| Renovate `platformAutomerge` | **`renovate.json5` から `platformAutomerge: false` を削除**(true 相当に戻す) | branch protection が唯一のゲートになるため、Renovate 側の待機ロジックへの依存が不要になる |
...
# 4. リポジトリ設定 Allow auto-merge を有効化
...
**後** に行う(順序を守らないと required checks なしで native auto-merge が
armed される穴が生まれる)。
...
以下は #116 実行前の現行(private + Free)の設定と検証コマンドの記録。
ADR 0004 本文の決定に対応する。#116 完了後は本節を削除してよい。
...
| Branch protection / rulesets | 設定不可(Free + private) | 上記「移行手順」参照 |
| Secret scanning + push protection | 設定不可(個人 private) | 上記「移行手順」参照 |
...
現行の automerge ゲートは「**すべての PR で CI(`test` + `cargo-deny`)が必ず走る**」
ことに依存している(`zizmor` は issue #79 当時は存在せず、ADR 0004 の required checks
...
- OSS 公開連作: (1/7) #113 LICENSE / (2/7) #114 本ドキュメントの public 前提化 /
  (4/7) #116 公開作業チェックリスト(本ドキュメントの手順を実行する)
