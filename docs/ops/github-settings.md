...
| (任意) `meguri/pr-review` を required check に追加 | **既定では未設定** | ADR 0008 の pr-reviewer レビューは既定で advisory(赤チェックは出すが human マージは止めない)。human 側も厳密ゲート化したい場合のみ、この context を required に足す。ただし付けるなら該当 kind のレビューを必ず ON にすること(`review.guard.impl` 等)— pr-reviewer が status を出さないと required check が永久 pending になりマージがハングする |
