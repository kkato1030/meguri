...
| guard(Plan) のラベル遷移 | guard(Plan) の settle は status/`<details>` に**加えて**ラベルも張り替える(現行 `spec_reviewer` と同一): clean → `spec-reviewing` 除去 + `spec-ready` 付与 / findings → `spec-reviewing` 維持。guard(Impl) はラベル遷移なし。この `spec-ready` が combined の spec_worker discover を駆動(ADR 0008 §3, finding 2) |
| guard(Plan)=OFF 時の label | planner が spec PR を **最初から `spec-ready`(spec-reviewing 非経由)で開く** — 内部 self-review だけが gate。OFF でも spec_worker が discover でき状態機械がデッドロックしない |
...
**guard(任意・kind 付き)**
- `src/engine/spec_reviewer.rs` → `src/engine/guard.rs` に一般化(kind パラメタ)。discover: Plan=`spec-reviewing` PR / Impl=`implementing` 相当の meguri PR(spec 系ラベルなし・CI green・head 未 guard・guard(Impl) 有効)。settle: `meguri/guard-review` status + 本文 `<details>` 追記(inline は出さない)。**kind=Plan は加えてラベル遷移**: clean→`spec-reviewing` 除去 + `spec-ready` 付与 / findings→`spec-reviewing` 維持(現行 `spec_reviewer` の張替を保持)。kind=Impl はラベル遷移なし。head マーカーで dedup(既存パターン)
...
- `src/engine/planner.rs` — separate では PR body を `Refs #N`(非クローズ)、combined では現行どおり。**guard(Plan)=OFF なら spec PR を最初から `spec-ready` で開く**(spec-reviewing 非経由)/ ON なら現行どおり `spec-reviewing`
- `src/engine/worker.rs` — separate 時に spec ライフサイクルを引き継ぐ: `execute_prompt` で `spec_rel_path(issue)` が worktree に在れば `spec_worker` の「Reviewed spec」注入 + 削除指示を流用(無ければ従来 normal フロー)。`verify_work` に「spec が残れば Err」の対称検証を追加。共通ロジックは `spec_worker` から小関数に切り出して共有(重複を避ける)
...
3. guard(Plan)=ON なら spec PR が現行相当にレビューされ、`meguri/guard-review` status + 本文 `<details>` が付く。guard(Impl)=ON なら実装 PR の head が独立レビューされる。**どちらも inline レビュースレッドを作らない**(同 FakeForge で fixer が反応しないことを確認)。
   - 3a. **guard(Plan) の clean は `spec-reviewing→spec-ready` を張り替える**(status/`<details>` だけでなく)。findings では `spec-reviewing` を維持。combined mode の spec_worker がこの `spec-ready` を discover して takeover することを確認(finding 2)。guard(Impl) は spec-* ラベルを触らない。
   - 3b. **guard(Plan)=OFF** のとき planner は spec PR を最初から `spec-ready` で開き(spec-reviewing 非経由)、guard 不在でも spec_worker が discover できる(デッドロックしない)。
