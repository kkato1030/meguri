...
この 2 点で「forge に新フィールド不要」は撤回。ただし追加は **bulk observe 1 クエリの中**に閉じ、
per-PR の追加 forge I/O は増やさない(ローカルな `succeeded_run_count` 読みを除く)。gh / fake も追随。
...
3. **act**:
   ...
   - `Agent(arm)` は **workqueue へ enqueue**: (a) backoff テーブルを見て `next_visible_at` が
     未来なら skip(§4 の backoff)、(b) claim marker が**自分が書いた・他 instance**のものなら
     skip(no-steal、§7)、(c) `queued` run を作る(active-run unique index が二重を弾く)。
...
- **消す = resync 側、観測駆動(level-triggered)**。毎 resync、その PR×arm の**症状が消えた**
  (conflict 解消 / CI 緑 / awaiting スレッド無し)か、**head が進んだ**(前進)ら
  `store.clear_backoff(project, item_key, arm)` で行を削除。コールバックに頼らず観測で必ず追いつく。
- 予算超過(`MAX_*_RUNS`)は backoff ではなく **parked(needs-human)への昇格**で、別物。

つまり **backoff は「結果が作り、観測が消す」**。これで `advance` が確実に呼ばれ、`next_visible_at` と
attempt が生成され、症状解消・head 更新で確実に clear される —— 受け入れ基準で検証可能になる。
...
- マーカー書式は `<!-- meguri:claim instance=<id> run=<run_id> -->`。head 非依存(1 work-item の
  claim は head 移動をまたぐ)。
- **単一 instance の排他の権威は、今日のとおり sqlite の active-run unique index**(atomic・偽装
  不能)。自分が書いた claim マーカーは forge から見える**射影**であり、Phase-4 の複数ホスト共有 DB で
  **クロスホストの権威に昇格**する(そのときも「自分が書いた」判定は残り、`instance` 欄が並行 meguri を
  区別する)。`meguri:working` は人間向けの表示射影(付け外しは続けるが権威ではない)。
- **no-steal の判定**: enqueue 前、観測コメントの中に *自分が書いた* claim マーカーで
  `instance` が**自分と異なる**ものがあれば skip。無い / 自分の instance なら進む。
- **release の契約**: run が終わったら、その claim comment を **node `id`(§1.5 で取得)で編集し
  無効化する**(tombstone に書き換え)。meguri が書いた comment なので確実に自分で消せる。編集に
  失敗しても無害: 次 resync で active run 無し + 症状解消を観測すれば、その PR×arm は素通しに戻る
  (level-triggered の回復)。
...
- **claim marker**: 追加コメント + release 時の**自分のコメント編集**(tombstone 化、§7)。
  `working` label は射影として付け外しを続けるので、旧バイナリ・人間の目には後方互換。
- **forge observe の拡張(§1.5)**: `observe_open_prs` の GraphQL に、各スレッドの最終 comment
  ...
  足す。schema ではなくクエリの拡張なので migration は不要・後方互換(gh/fake の read が広がるだけ)。
...
4. **claim / no-steal(芯3・f3)**: (a) *自分が書いた*(`viewerDidAuthor==true`)他 instance の
   ...
   dispatch 可。(d) release で claim comment が `id` 指定で tombstone に編集される。FakeForge に
   author 付きマーカーコメントを積んで連結検証。
5. **backoff(f4)**: (a) リトライ可能な run 結果(transient 失敗 / 症状残存の成功)で
   `advance_backoff` が呼ばれ `next_visible_at = base*2^attempt` を指数で置く。(b) due 前の PR×arm は
   enqueue されない(run が作られない)。(c) 症状解消 / head 前進を観測した resync で `clear_backoff`
   され行が消える。(d) sqlite なので restart をまたいで生存。`next_step` に `RequeueAfter` は無い。
...
- `src/engine/mod.rs`(3 fixer は `default_loops()` に**登録を残す**が discover は空(f1)、
  `pr_is_touchable` の claim 判定を marker(自分書き)へ)
- `src/engine/scheduler.rs`(`queued` run を優先度順に dispatch、backoff enqueue ゲート、
  resync sweep 呼び出しを継ぐ)
- `src/store/migrations/0016_reconciler_backoff.sql`(新規)+ `src/store/reconciler.rs`
  (アクセサ新規: `advance_backoff` / `clear_backoff` / read)
...
4. dispatch が workqueue + resync で動く: `queued` run が優先度順(マージ近接)に出る、parked は run を
   作らない。**backoff(f4)**: リトライ可能な run 結果が `advance_backoff` を呼んで `next_visible_at` を
   指数で置き、due 前は enqueue されず、症状解消 / head 前進の観測で `clear_backoff` され、restart を
   またいで生存する。
...
7. claim marker(f3): 他 instance の**自分が書いた**claim を dispatch しない(no-steal)一方、
   **第三者の偽マーカーは無視して dispatch を止めない**property test が緑。release で claim comment が
   `id` 指定で無効化される。`meguri:working` は表示射影として付け外しされる。
