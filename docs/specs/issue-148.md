...
- **場所**: not-before → cadence の順で、`has_unresolved_blockers` と同じ discover 内の層に挿す(claim より前)。スキップはラベルもコメントも書かない(dependencies ブロックと同じ流儀)。
...
- **`src/tasks.rs`**:
  ...
  - `LabelTaskSource::discover`: `has_unresolved_blockers` の直前に not-before(本文マーカー)と cadence(`self.cadence_rules` × `store` の窓内カウント)ゲートを追加。cadence 通過時、issue が該当ラベルを持つなら `Task` に cadence バケツを載せる。cadence rule は `project.cadence` を source が保持。
  - `LocalTaskSource::discover`: `not_before` フィールドで not-before ゲート(cadence は github 専用なので無し)。
  - 消化カウント: 下記 `Store::cadence_consumed` を使い、`残枠 = max - consumed` を FIFO で配る(残枠超過分は返さない)。
...
- **`src/store/runs.rs`**:
  ...
  - `cadence_consumed(project_id, label, window_start_ts) -> i64`: `SELECT COUNT(*) FROM runs WHERE project_id=? AND cadence_label=? AND created_at >= ? AND status != 'skipped'`。
...
- **`src/tasks.rs` 内 tests**: 注入 clock 付きで discover のゲート順序と cadence の残枠配布(FIFO)。
