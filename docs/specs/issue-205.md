...
一行で書ける。**すでに store に emit されているイベント(`turn.awaiting_human` /
`escalation.raised` / 新設の `schedule.failed`)を、config の allowlist で選び、既存の
`src/notify/` シンクから webhook へ流す。**
...
- `events: Vec<String>`(default `["awaiting_human"]`)。**後方互換の要**: 既存 config
  (webhook_url あり・events 無し)は default で現行と同一挙動になる。escalation / schedule を
  足したい人だけ明示列挙する。
