...
大きな issue に meguri:plan
  ...
  → spec PR(spec-reviewing → 既存 review ゲート → spec-ready
      + clean 承認時に head_sha へ meguri/pr-review success status(既存動作))
  → materialization sweep: spec-ready かつマーカー付き "open" PR のうち、
      現 head_sha に meguri/pr-review success がある(= 承認された head のまま)ものだけを実行
      (承認後に head が動いた PR は spec-ready → spec-reviewing に戻して再レビューへ)
...
- **materialization sweep(新規)**: `spec-ready` かつマーカー付き かつ head branch が issue を
  encode する **open** PR を拾う。skip 条件は二点 — **提案 PR が open でない(= closed/merged)**、
  および **現 head が承認済みでない**(guard(plan) on のとき。下記「承認 head と読む head の一致」 —
  この場合は skip に加えて `spec-reviewing` へ戻す)。materialization の唯一の commit point は
  「提案 PR を close する」ことなので、PR が open である限り毎掃引が全 materialization 手順を冪等に
  再実行する(下記 §2)。「子マーカーが全部揃った」を完了とはみなさない — それは「子作成済み」で
  あって「親 tracking 化 + PR close 済み」ではない。
- **spec-worker discover**: マーカー付き PR を**除外**(実装 takeover しない)。
- **handoff sweep**: マーカー付き PR を**除外**(`speccing → ready` 張替をしない)。
- **pr-reviewer(plan)(旧 guard(Plan))**: **コード無変更**。分解提案も通常 spec と同じく
  `spec-reviewing → spec-ready` を張り替え、clean 承認時に head_sha へ `meguri/pr-review` success の
  commit status を書く(`PR_REVIEW_STATUS`、`src/engine/pr_reviewer.rs` — 既存動作)。この per-head
  status を materialization が「承認の証跡」として消費する(下記「承認 head と読む head の一致」)。
  カバレッジのレビューは spec 本文を読む prompt/内容の問題で、機構は変えない。

### 承認 head と読む head の一致(head-motion gate — レビュー済みでない children を実体化しない)

`spec-ready` は**ラベル**であり sha に紐付かない。一方 pr-reviewer(plan)の discover は
`spec-reviewing` PR しか見ない(`src/engine/pr_reviewer.rs`)ため、承認(spec-ready 付与)後に
head へ push されても再レビューには戻らない。separate delivery では spec-ready の提案 PR にも
fixer-family が push しうる。この隙間を放置すると、**レビューされていない children を不可逆に
issue 化できてしまう**。そこで materialization に「承認された head = 読む head」の一致条件を入れる:

- **承認の証跡 = 現 head_sha 上の `meguri/pr-review` success status**。pr-reviewer が clean 承認時に
  書く既存の per-head commit status をそのまま消費し、sweep は
  `commit_status(head_sha, PR_REVIEW_STATUS) == Success` のときだけ materialize に進む。これは
  auto-merger の arm gate(ADR 0008 §5 — impl PR は**現 head**に pr-review success があるときだけ
  merge する)と**同型**のゲートを plan 側に対称に置くもの。status は sha に固有なので「レビューされた
  内容」と機械的に一致し、新しい marker・新しい書き込みは一切増えない(pr-reviewer / planner とも
  無変更。`commit_status` は Forge trait に既存)。
- **不一致(現 head に success が無い)なら warn + skip し、`spec-reviewing` に戻す**(再承認経路):
  sweep が `spec-reviewing` を付与してから `spec-ready` を除去する(途中失敗で両ラベルが残っても
  次掃引が張替をやり直す、安全側)。pr-reviewer の discover は「`spec-reviewing` かつ現 head に
  status 無し」を既に拾うので、新しい head は**既存機構だけで**自然に再レビューされる — clean なら
  spec-ready(+ 新 head への success status)に復帰し、次掃引が materialize する。専用の駆動役は
  誰も要らない。
- **`guard.plan = false` のときは条件を課さない**。このモードでは planner が spec PR を直接
  `spec-ready` で開き(ADR 0008 §3 — 内部 self-review が唯一のゲート)、status を書く者がいない。
  承認ゲート自体をオプトアウトした運転なので、head 一致条件も同じスイッチで外れる(課すと誰も
  status を書けず deadlock する)。
- 人間が(guard on のまま)手動で `spec-ready` を貼った場合も、現 head に success が無ければ一度
  `spec-reviewing` に戻り pr-reviewer の再レビューを経る — 「承認 head の証跡が status として残る」
  ことを優先する。ゲートを飛ばしたい運転には `guard.plan = false` という既存の手段がある。
- 本条件は**分解提案(マーカー付き)PR に限る**。通常 spec の spec-worker takeover は成果物(実装)が
  impl 側のゲート(self-review / CI / pr-review / auto-merger の arm gate)を別途通るため、
  本 issue では触らない。
...
- **回復経路**: branch 上の spec を直して push すると head が動くので、guard(plan) on では承認 head
  条件(上記「承認 head と読む head の一致」)により掃引が `spec-reviewing` に戻し、pr-reviewer の
  再レビュー(clean)を経て spec-ready に復帰した新 head を次掃引が読み直す。guard off ならそのまま
  次掃引が読み直す。いずれも掃引自体は状態を持たない。放置する判断なら人間が提案 PR を close すれば
  discover から外れる。
...
- `show_file_at_ref(git_ref, path) -> Result<String>`(内部 `git show <ref>:<path>`。ref は
...
- **D. 即時 `status: decompose`(#24)を残し、reviewed 経路と併存させる**。→ **却下**。分解機構が
  ...
  (filing 中核は再利用)。
- **E. 承認 head の担保を「承認時 head を PR body の hidden marker に記録」または「head 変化を検知して
  spec-reviewing に戻す専用機構」で行う**(head-motion gate の対案)。→ **却下**。承認時 head の
  per-sha 記録は `meguri/pr-review` commit status として**既に forge にある**(pr-reviewer の dedup
  key 兼 auto-merger の arm gate)。marker 案は同じ事実を PR body に二重化し、pr-reviewer settle に
  decompose 専用の書き込みを漏らす上、body は人手・他ループの編集(reconcile は body 編集を検知して
  再処理する)にさらされ、証跡として status より弱い。「head 変化の検知」を単独機構にする案は
  「前回 head」の記憶が要り、状態を持たない掃引の設計に反する。既存 status の消費 + 不一致時の
  ラベル戻しなら**新規書き込みゼロ・無状態**で、再レビュー駆動も pr-reviewer の既存 discover に乗る。
  「人間承認 marker」(status 無しでも人間が head を承認したと記す)も検討したが、ゲートを外したい
  運転は `guard.plan = false` で既に表現できるため導入しない。
...
- events(sqlite、`meguri logs`): `issue.decompose_proposed`(spec PR 開く)・
  ...
  `issue.materialize_deferred`(reserve 済み未リンクの子を再作成せず次掃引へ送った時)・
  `issue.materialize_head_stale`(現 head に承認 status が無く `spec-reviewing` へ戻した時
  `{parent, pr, head}` — 承認後に head が動いた事象の可視化)。既存 `issue.decomposed` の命名に倣う。
...
- **単体(loop 単位)**: FakeForge + マーカー付き PR で materialization sweep の discover フィルタ
  ...
  `close_pr` を検証。
- **head-motion gate(finding 直結 — 承認後に head が動いた場合)**: guard(plan) on の FakeForge で
  (commit status は既存の `set_commit_status` / `commit_status` を再利用):
  1. spec-ready + マーカー付きだが**現 head に `meguri/pr-review` success が無い** PR は、issue を
     1つも作らず `spec-ready → spec-reviewing` に張り替わる(warn + `issue.materialize_head_stale`)。
     張替は冪等(再掃引しても事故らない)。
  2. その後 pr-reviewer 相当の再承認(新 head へ success status + spec-ready 復帰)を模すと、
     次掃引が materialize する(再承認経路の end-to-end)。
  3. 現 head に success がある場合だけ drive に進み、読む spec はその sha の内容である(掃引中に
     tip が動いた場合は skip)。
  4. `guard.plan = false` では status 無しでも materialize する(条件が同じスイッチで外れる)。
...
9. guard(plan) on のとき、承認(spec-ready)後に head が動いた提案 PR は materialize され**ない**:
   sweep は現 head_sha の `meguri/pr-review` success を要求し、無ければ issue を一切作らず
   `spec-ready → spec-reviewing` に戻す。pr-reviewer が新 head を再レビューし、clean で spec-ready に
   復帰した head だけが実体化される。`guard.plan = false` では条件を課さない(承認ゲートごと
   オプトアウト、ADR 0008 §3)。
10. 既存テスト(planner / spec_worker / handoff / scheduler)が非破壊で通る。
