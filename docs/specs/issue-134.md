...
大きな issue に meguri:plan
  ...
  → spec PR(spec-reviewing → 既存 review ゲート → spec-ready
      + clean 承認時に head_sha へ meguri/guard-review success status(既存動作))
  → materialization sweep: spec-ready かつマーカー付き "open" PR のうち、
      現 head_sha に meguri/guard-review success がある(= 承認された head のまま)ものだけを実行
...
- **guard(plan)**: **コード無変更**。分解提案も通常 spec と同じく
  `spec-reviewing → spec-ready` を張り替え、clean 承認時に head_sha へ `meguri/guard-review` success の
  commit status を書く(`GUARD_STATUS`、`src/engine/guard.rs` — 既存動作)。この per-head
...
`spec-ready` は**ラベル**であり sha に紐付かない。一方 guard(plan)の discover は
`spec-reviewing` PR しか見ない(`src/engine/guard.rs`)ため、承認(spec-ready 付与)後に
...
- **承認の証跡 = 現 head_sha 上の `meguri/guard-review` success status**。guard が clean 承認時に
  書く既存の per-head commit status をそのまま消費し、sweep は
  `commit_status(head_sha, GUARD_STATUS) == Success` のときだけ materialize に進む。これは
  auto-merger の arm gate(ADR 0008 §5 — impl PR は**現 head**に guard-review success があるときだけ
  merge する)と**同型**のゲートを plan 側に対称に置くもの。status は sha に固有なので「レビューされた
  内容」と機械的に一致し、新しい marker・新しい書き込みは一切増えない(guard / planner とも
...
- **不一致(現 head に success が無い)なら warn + skip し、`spec-reviewing` に戻す**(再承認経路):
  ...
  次掃引が張替をやり直す、安全側)。guard の discover は「`spec-reviewing` かつ現 head に
...
- 人間が(guard on のまま)手動で `spec-ready` を貼った場合も、現 head に success が無ければ一度
  `spec-reviewing` に戻り guard の再レビューを経る — 「承認 head の証跡が status として残る」
  ことを優先する。ゲートを飛ばしたい運転には `guard.plan = false` という既存の手段がある。
- 本条件は**分解提案(マーカー付き)PR に限る**。通常 spec の spec-worker takeover は成果物(実装)が
  impl 側のゲート(self-review / CI / guard-review / auto-merger の arm gate)を別途通るため、
  本 issue では触らない。
...
- **回復経路**: branch 上の spec を直して push すると head が動くので、guard(plan) on では承認 head
  条件(上記「承認 head と読む head の一致」)により掃引が `spec-reviewing` に戻し、guard の
...
- **E. 承認 head の担保を「承認時 head を PR body の hidden marker に記録」または「head 変化を検知して
  ...
  per-sha 記録は `meguri/guard-review` commit status として**既に forge にある**(guard の dedup
  key 兼 auto-merger の arm gate)。marker 案は同じ事実を PR body に二重化し、guard settle に
  ...
  ラベル戻しなら**新規書き込みゼロ・無状態**で、再レビュー駆動も guard の既存 discover に乗る。
...
- **head-motion gate(finding 直結 — 承認後に head が動いた場合)**: guard(plan) on の FakeForge で
  ...
  1. spec-ready + マーカー付きだが**現 head に `meguri/guard-review` success が無い** PR は、issue を
     1つも作らず `spec-ready → spec-reviewing` に張り替わる(warn + `issue.materialize_head_stale`)。
     張替は冪等(再掃引しても事故らない)。
  2. その後 guard 相当の再承認(新 head へ success status + spec-ready 復帰)を模すと、
...
9. guard(plan) on のとき、承認(spec-ready)後に head が動いた提案 PR は materialize され**ない**:
   sweep は現 head_sha の `meguri/guard-review` success を要求し、無ければ issue を一切作らず
   `spec-ready → spec-reviewing` に戻す。guard が新 head を再レビューし、clean で spec-ready に
