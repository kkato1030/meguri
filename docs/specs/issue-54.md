...
- **claim-by-key であって claim-next ではない。** 現行の scheduler は「discover → target ごとに run 作成 → drive 先頭で claim」という構造で(`scheduler.rs:83-121`)、これを保つ。アトミック性は claim 単体が担保する: sqlite では `UPDATE tasks SET status='claimed', claimed_by=?, ... WHERE id=? AND status='queued'` の affected rows 判定、Phase 4 の Postgres では同型の `UPDATE ... WHERE status='queued' OR lease_until < now() RETURNING *`。lease 失効条件を足すだけで契約は変わらない。
...
- unique index は 2 本の部分インデックスに分ける: `runs_active_issue (project_id, loop_kind, issue_number) WHERE active AND issue_number IS NOT NULL` と `runs_active_task (project_id, loop_kind, task_id) WHERE active AND task_id IS NOT NULL`。既存データはそのまま前者に載る。
...
- `repo_slug` を `Option<String>` にする。ロード時検証: `mode != "local"` なら必須。`mode = "local"` では `deliver = "pr"` を設定エラーにする(push 先がない)。デフォルトは mode を問わず `deliver = "pr"`(local だけ実質 branch/patch の二択)。
...
- `src/store/tasks.rs` の unit test: claim の原子性(連続 claim で二度目が None)、状態遷移、needs_human の reason 保存。
...
- `src/engine/mod.rs` — `Deps.task_source`、`forge: Option<_>`
