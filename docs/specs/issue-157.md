...
- 直接マージヘルパ:draft なら `mark_pr_ready` → `merge_pr(pr.number, strategy, head_sha)` → 監査用に merged コメント + `pr.automerge_merged` イベント emit(既存の `merged_comment` / event を再利用)。`--match-head-commit` により head 移動時は GitHub/fake が拒否(TOCTOU)。
...
- orchestrator: 適格 PR で `pr_mergeable = Mergeable` → `merge_pr` が呼ばれ PR が merged になる(`set_pr_mergeable` / `merged` を検証)。
- ブロックラベル付き → マージされない。
- 未解決スレッドあり → マージされない。
...
2. orchestrator モードで、適格かつ `MERGEABLE` の meguri PR が設定 strategy で直接マージされる(arm もマーカーも作らない)。
