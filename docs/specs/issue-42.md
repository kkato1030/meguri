...
| **Transient**(429/5xx 等でスナップショットが取れない) | — (状態不明) | **何もしない**(no-op/retry。次掃引で再試行。needs-human を貼らない=分類できないまま fixer を締め出さない) |
| **Stuck**(スナップショットが取れていて、上記いずれの担い手も無いまま arm-since が閾値超) | **どのループも拾わない** | **唯一エスカレーションする分類**: `meguri:needs-human` + 状況コメント |
...
→ Stuck 判定 = 「arm 済み・未 merged・**スナップショットが取れていて**・Conflict でも RedCI でも HumanDisabled でもない状態が、arm-since から `STALE_AFTER` 超」。sqlite に一切持たず、meguri をいつ kill しても forge から再導出でき、受け入れ条件「マーカーコメントだけで再起動後も watch が継続」を(専用マーカーすら使わず)満たす。

> **critical(修正):** TransientError(429/5xx でスナップショットが取れない)は **Stuck に畳んではならない**。スナップショットが取れていない間は、その PR が実際には Conflict(conflict-resolver 対象)なのか RedCI(ci-fixer 対象)なのか判定できない。にもかかわらず「取れないまま `STALE_AFTER` 超」という時間条件だけで `meguri:needs-human` を貼ると、API 障害や一時的な取得失敗の後にラベルが残り、conflict-resolver / ci-fixer がその PR を discover から除外して、この PR の中心不変条件(Conflict / RedCI には needs-human を貼らず fixer ループを塞がない)を破ってドリフトを永久にデッドロックさせる。したがって **Transient は独立分類とし、常に no-op/retry**(次掃引で再試行)にする。エスカレーションは「状態を positive に読めて、かつ Conflict / RedCI / HumanDisabled のいずれでもないと**確認できた**」場合の Stuck だけに限る。掃引ごとのリトライは自動なので明示のリトライカウンタは不要。`STALE_AFTER` の壁時計は、あくまで**読めている Stuck 状態**の継続時間にのみ適用する(取得失敗の連続時間には適用しない)。
...
- [ ] `Classify` の単体テスト: Merged / Conflict / RedCI / HumanDisabled / Healthy(UNSTABLE) / Stuck / Transient の各スナップショットが正しい分類になる。とくに **Transient は arm-since が閾値超であっても Stuck にならず Transient のまま**(時間条件で Stuck に昇格しないこと)。
- [ ] FakeForge での e2e:
  ...
  - **Transient(スナップショット取得が 429/5xx で失敗)→ arm-since が閾値超でも needs-human もコメントも付けない**(no-op/retry)。取得が回復して Conflict/RedCI だった場合も needs-human を貼らないこと(fixer をデッドロックさせない)。
  - Stuck(スナップショットが取れていて、arm-since が閾値超・どのループも拾わない BLOCKED)→ needs-human + コメント 1 回。二掃引目は needs-human で素通り(冪等)。
