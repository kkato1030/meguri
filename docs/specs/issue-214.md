...
7. **findings 上限。** round 1 parallel prompt に「1本あたり最大 K 件」。K は定数(既定を置き、
   必要なら後で config 化)。
8. **スロット重み。** `[[review.reviewers]]` を持つ run は round 1 で N 本同時に走るため、
   `run_weight` を並列本数を織り込んだ値にする(§scheduler)。
9. **launch mode。** reviewer の launch mode は self-reviewer profile から解決(現行どおり)。
   one-shot なので `direct` 推奨(ADR 0012)。並列でも lane キーを一意化すれば pane モードでも衝突しない。

### 未決(review で詰めたい)

- **K(reviewer あたり findings 上限)の既定値** と config 化の要否。
- **スロット重み式**:`run_weight = max(advisor_weight, N_reviewers)` か、単純に `1 + N` か。
  advisor(execute 中に並行)と review(self-review 中に並行)は時間帯が重ならないので max が妥当だが、
  実装単純さと安全側(過剰予約)のどちらを取るか。
- **reviewer profile の解決失敗時**の扱い(1本 detect 失敗 → その1本を落として残りで続行 or run 失敗)。
  recall 目的なので「落として続行 + イベント記録」を推す。
...
- **完了コントラクト**:per-turn `result-<turn_id>.json` は並列 review turn 限定。既存の全経路
  ...
  コントラクトは変わらない。
- **rollback**:`[[review.reviewers]]` を消す(or `review.enabled = false`)で単独経路へ即座に戻る。
  in-flight run が並列 round 1 の途中で rollback された場合、round 1 は台帳へ畳む前なら再実行で単独経路に
  落ちる(review は read-only で冪等)。DB schema 変更はない。
...
- **単体(FakeMux/FakeForge)**:
  ...
  - scheduler:`reviewers` 有りの run が正しい重みを予約し `max_concurrent` を越えない。
