...
- **決定 D1b — transcript locator の契約と profile の一致規則**(finding f4):
  ...
  - **未対応レイアウトの安全な fallback**: profile の CLI が既知レイアウトを持たない
    (`transcript_len` が `None`)場合、G1 は resume を止めず素通しし、`pane.resume_size_unknown`
    `{profile}` を emit する。サイズで測れなくても **G2(quiet カウンタ)が backstop** として
    context 100% を有界回数で断つ。「測れないなら無条件 fresh 化」はしない(健全な resume まで
    捨ててしまうため)。
...
- **決定 D2 — 無応答を終端させるが、新 variant は `TurnOutcome` に足さない**(finding f3):
  `await_completion`(pane mode)の戻り型を新設の内部 enum
...
- **決定 D5b — nudge は「在否確認と送信を分離しない」原子的 API にする**(finding f2):
  「`agent_present` で確認 → `send_line` で送信」は2回の mux 呼び出しなので、確認直後に agent が
  exit して pane が shell に戻ると shell へ nudge を打ち込む TOCTOU が残る。これを塞ぐため、
  nudge 専用メソッド `Multiplexer::nudge(&self, pane, text) -> MuxResult<NudgeOutcome>`
  (`NudgeOutcome ∈ {Delivered, AgentAbsent}`)を足し、`await_completion` の nudge は `send_line`
  ではなくこれを使う。**契約**: 実装は「agent が pane の前面プロセスである」ことと送信を
  **一つの操作**として行い、agent 不在なら `text` を**届けない**。呼び出し側の TOCTOU を無くす。
  - **herdr**: native。agent integration 宛ての送信を使い、agent が detach していれば no-op/失敗
    (サーバ側で原子的)。
  - **tmux**: agent 宛て送信は無いので、`send-keys` を単一の `if-shell` でガードする
    (`tmux if-shell -t <pane> '<pane_current_command が shell でない判定>' 'send-keys …'`)。
    判定と送信が tmux の1コマンド評価に収まり、確認→送信の間に別プロセスが割り込む窓を潰す。
    shell と判定されたら send-keys は実行されず `AgentAbsent` を返す。
...
- `src/mux/mod.rs` — `Multiplexer::agent_present`(既定 `Ok(true)`)と
  `Multiplexer::nudge(pane, text) -> MuxResult<NudgeOutcome>`(原子的・既定は
  `send_line` へ委譲)を追加(G3/D5・D5b)。
- `src/mux/herdr.rs` / `src/mux/tmux.rs` / `src/mux/fake.rs` — `agent_present` と `nudge` 実装
  (tmux は `if-shell` ガード付き `send-keys`)(G3)。
- `src/turn/mod.rs` — 内部 enum `AwaitOutcome`(`AgentQuiet{tail}`/`AgentAbsent{tail}` を含む)を新設し
  `await_completion` の戻り型に。nudge 撃ち尽くし時の返却 + 在否チェックで `nudge` を使用
  (G2/D2・G3(b))。**`TurnOutcome`(公開)は不変**。`await_completion_direct` も `AwaitOutcome` を
  返す(quiet/absent は生成しない)。
...
- **unit**:
  ...
  - `nudge` の原子性契約(f2): FakeMux で agent-absent 時に `NudgeOutcome::AgentAbsent` を返し
    `sent_lines` が空のまま。tmux 実装は `if-shell` ガードを通ること(mux_tmux テストに追加)。
