...
- **D9 `agent_session.cleared` の reason**: `transcript_oversize` / `quiet_loop` /
  `agent_absent`(既存の「resumed executor died…」に加える)。`discard_pane` が session を消す
  ときに出す。
- **D10 再 dispatch を本当に fresh spawn へ進める(f3 修正・最重要)**: `ensure_pane` は
  ...
  `mux_pane_id` が残れば同じ pane を adopt し、次の trigger が素のシェルへ入る。
  **注意(既存挙動の落とし穴)**: `reaper::release_pane` は行の session id を読まず、worktree の
  transcript を `latest_session_id` で**再走査して save し直す**(reaper.rs:498–510)。恒久 400 や
  素のシェルの transcript はディスクに残っているので、release_pane を使うと消したはずの
  session id が即復活し、次 dispatch がまた resume してしまう。よって release_pane は使わない。
  専用の teardown を規定し、二重で塞ぐ:
  1. **teardown(主機構、新ヘルパ `discard_pane`)**: quiet ローテート(strike≥2)/
     transcript-oversize / `Absent → PaneDied` の各経路で、**この順**で行う —
     ① live pane を `kill_pane`、② `mark_pane_reclaimed`、③ 最後に `save_pane_session(None)` +
     `update_run_agent_session(None)`。transcript 再走査による save を挟まないので、行は
     `mux_pane_id = NULL` かつ `agent_session_id = NULL` で確定する。次 dispatch は adopt
     できず、session も無いので**必ず fresh spawn**。`pane.discarded {reason}` を emit。
     (`Absent → PaneDied` は `record_agent_session` が session を None にするだけで pane を
     kill/reclaim しないため、この経路でも `discard_pane` を必ず通す。)
  2. **adopt ゲート(防御)**: `ensure_pane` の adopt 分岐に「live pane の `agent_state` が
     `Absent` なら adopt せず `discard_pane` して再 spawn へ落とす」条件を足す。teardown が
     漏れても素のシェルを adopt しない。
  - strike 1(session 保持)の再試行は、pane が生きていれば従来どおり adopt して同一
    session を resume。context 一過性ならこれで回復、駄目なら strike 2 で `discard_pane`。
- **D11 `session_root` は pinned lane profile から解決する(f4 修正)**: transcript を触る箇所は
  今 `session_root(&deps.config.agent)` 固定で(gate 未実装、`record_agent_session` flow.rs、
  reaper.rs:498)、named profile の `session_dir` や custom CLI の transcript を見失う。3箇所の
  locator を **pinned profile 起点**に揃える:
  - **gate(`ensure_pane` / `spawn_direct_process`)**: `agent_session::session_root(&lane.profile)`。
    lane.profile は既に解決済み。
  - **`record_agent_session`**: lane profile を引数でスレッドし `session_root(&lane.profile)`。
  - **reaper のセッション退避(`release_pane_record`)**: その pane を最後に持っていた run の
    pinned profile を lookup して root を引く。**profile を解決できない**(該当 run 無し等)
    ときは従来どおり `deps.config.agent` に fallback(退化するだけで壊れない)。
  - **未対応レイアウトの fallback**: transcript を特定できない時は gate を**素通り**(resume を
    止めない)にし `transcript.locate_failed` を emit。oversize の取りこぼしは quiet-strike が backstop。
...
- `src/engine/flow.rs`
  - 新ヘルパ `discard_pane(deps, issue, lane, reason)`: ① `kill_pane` → ② `mark_pane_reclaimed`
    → ③ `save_pane_session(None)` + `update_run_agent_session(None)` の順で、transcript を
    再走査せずに行を `mux_pane_id=NULL` / `agent_session_id=NULL` に確定させる(D10-1)。
    `reaper::release_pane` は transcript を再走査して session を save し直すので使わない。
  - `ensure_pane`: (a) resume 前に transcript サイズゲート — root は **lane profile** から
    解決(D11)、超過なら `discard_pane`(`agent_session.cleared: transcript_oversize`)して
    fresh spawn。(b) adopt 分岐に `Absent` ゲート(D10-2、Absent なら `discard_pane` して再 spawn)。
  - `spawn_direct_process`: 同じゲート(root は lane profile から解決)。
  - `resumed_pane_survives`: `Absent` を「生存せず」に。
  - `run_turn_in`: `AgentQuiet` を strike ハンドラへ。bump →(strike≥2 で `discard_pane`,
    reason `quiet_loop`)→ strike≥3 は `Err(NeedsHuman(sanitize_pane_tail(tail) 同梱))`、
    それ未満は `Ok((AgentQuiet, id))`。
  ...
  - `Absent → PaneDied` 経路でも `discard_pane`(reason `agent_absent`)を通して再 adopt を防ぐ
    (`record_agent_session` は session を消すだけで pane を kill しないため)。
  - tail を `NeedsHuman` 理由へ埋める箇所で `sanitize_pane_tail` を通す(D12)。ローカルイベント
    (`turn.agent_quiet`)には生 tail、Forge/通知にはサニタイズ版だけが届く。
- `src/engine/reaper.rs`: `release_pane_record` のセッション退避 `session_root` を、pane を最後に
  持っていた run の pinned profile から引く。解決不能なら `deps.config.agent` に fallback(D11)。
...
```
discard_pane(issue, lane, reason) = kill_pane → mark_pane_reclaimed → save_pane_session(None)
                                     (transcript を再走査しない。release_pane は使わない)
resume 要求(ensure_pane / spawn_direct_process)
  ├ adopt: live pane が Absent → discard_pane して再 spawn へ(D10-2)
  └ transcript_len(root=lane profile) > 実効閾値 ?
        → discard_pane(cleared: transcript_oversize) → fresh spawn
await_completion(pane)
  ├ AgentState::Absent            → PaneDied(reason: agent_absent)  ※nudge しない
  │                                  → run_turn_in が discard_pane(agent_absent)
...
run_turn_in が AgentQuiet を受けたら:
  ...
  strikes >= 2 → discard_pane(cleared: quiet_loop)      ← 次 dispatch で必ず fresh spawn
...
全経路が `discard_pane` を通るので、次 dispatch は `mux_pane_id` の残留で素のシェルを再 adopt
することも、消したはずの session id が transcript 再走査で復活することもない(f3)。
...
- events: `turn.agent_quiet {strikes, tail}`(生 tail はローカル sqlite 限定、f5)/
  `discard_pane` が出す `pane.discarded {reason}` + `agent_session.cleared {reason}`
  (reason ∈ `transcript_oversize` / `quiet_loop` / `agent_absent`)/
  `turn.pane_died {reason: "agent_absent"}` / `transcript.locate_failed`(f4)/
...
- **flow(FakeMux、resume_test.rs 系)**:
  ...
  - quiet ストライク: 1→再試行 / 2→`discard_pane` で行が `mux_pane_id=NULL` かつ
    `agent_session_id=NULL` になり、**transcript がディスクに残っていても**次 spawn が
    `--resume` なしの full prompt になること(f3 の要:release_pane の再走査 save で session が
    復活しない回帰テスト)/ 3→`NeedsHuman`(reason にサニタイズ済 tail)。
  - `Completed` で `quiet_strikes` が 0 に戻る。
  - **adopt ゲート(f3)**: `mux_pane_id` が生きていても `agent_state == Absent` なら adopt せず
    `discard_pane` → 再 spawn になること。
