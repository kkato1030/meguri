...
- 既存テストが緑のまま。エスカレーションコメントの投稿先は `pr_comment`（→ `comments_of`）の
  ままなので、`comments_of` を読む ~30 テスト（ci_fixer / pr_reviewer / reconcile / merge_watch /
  auto_merge / planner / `spec_fixer` の budget 枯渇テスト等）は読み替え不要（f7）。`spec_fixer`
  budget 枯渇テストのコメント数・イベント数・通知数も不変（各1件）。
...
6. **順序と失敗セマンティクス**（`park_pr` 内、**全経路で `working` を解放する**）:
   ...
   2. `head=Some(h)` の場合、**`pr_comments_meta(pr)` を live read** し、**自著
      （`PrComment.viewer_did_author`）** かつ `escalation_marker(h, reason)` を含むコメントが
      あれば、`remove_pr_label(working)` してから `Deduped` を返す（再投稿しない）。第三者が同じ
      marker を先に投稿しても自著判定で無視する（f4、ADR 0027 の claim marker と同じ立て付け）。
      `viewer_did_author` は既存の `Forge::pr_comments_meta` が返す `PrComment` の項目で、
      `GhForge` が GraphQL の `viewerDidAuthor` から埋めている（`src/forge/gh.rs:145`）ため、
      **`Forge`/`GhForge` に新メソッドは不要**。read 自体が失敗したら投稿へ進む — 稀な read 障害で
      高々1件重複する方が握りつぶすより安全（ラベルは read-after-write 済み）。← dedup 経路の
      claim 解放（f1）＋ 自著判定（f4）
   3. `remove_pr_label(working)`（best-effort, `let _`）。
   4. **`pr_comment`** で `comment + "\n\n" + marker` を投稿し `Posted` を返す。エスカレーション
      コメントは issue #176 以来 `pr_comment`（→ FakeForge の `comments` ビュー）に載せる human
      向けチャネルで、既存の ~30 テストが `comments_of` で読む。投稿先はこれを維持する（`comment_pr`
      に変えない — f7）。dedup が読む `pr_comments_meta` からも見えるよう、FakeForge 側で
      `pr_comment` を両ビューへ記録するよう直す（下記 item 9 / f5）。
...
9. **FakeForge のテスト支援と modeling 修正**:
   - **`pr_comment` を両ビューへ記録する（f5/f7）**: 現状 `pr_comment` は legacy の `comments` に
     だけ積み、dedup が読む `pr_comments`（`PrComment` 配列）には載らない。`pr_comment` が
     `comments`（`comments_of` 用）と `pr_comments`（`pr_comments_meta`/dedup 用、
     `viewer_did_author = true`・`created_at = now`）の**両方**へ積むよう直す。これで投稿先を
     `pr_comment` のまま維持しつつ（既存 `comments_of` テストが緑）、dedup の live read からも
     見える。実 forge では会話コメントは1系統なので、この統一はむしろ忠実。
     （`comment_pr` は marker 専用のまま。arm/claim marker テストへの波及を避けるため触らない。）
   - `fail_pr_label(pr, count)`: `add_pr_label` が指定 PR で N 回失敗してから成功する（既存の
     `fail_comment` / `fail_merge_state` と同じ Mutex フィールド様式。失敗回数を数える
     `Mutex<HashMap<i64, u32>>`）。PR ラベル書き込み失敗の注入に使う。
   - **`fail_label(issue, count)`（f6）**: `add_label`（issue 側、`add_pr_label` とは別メソッド）が
     指定 issue で N 回失敗してから成功する。同じ Mutex 様式。f2 の issue-native read-after-write
     回帰テストに使う（既存の支援は PR ラベルしか失敗させられない）。
   - `push_pr_comment_from_other(pr, body)`: `viewer_did_author = false` の `PrComment` を
     ...
     （`pr_comment`/`comment_pr` は常に自著なので、非自著コメントを別ルートで seed する）。
...
- `src/engine/escalation.rs` — `park_pr` / `Escalated` / `escalation_marker*` を追加。
  ...
  投稿は既存どおり `pr_comment`。`escalate_issue` に read-after-write（失敗時は `working` 解放）
  を追加。
- `src/forge/mod.rs` / `src/forge/gh.rs` — **変更なし**。dedup が使う `viewer_did_author` は
  既存の `pr_comments_meta` / `PrComment` が提供済み（`gh.rs:145` が `viewerDidAuthor` を充填）。
...
- `src/forge/fake.rs` — `pr_comment` を両ビュー記録に直す（f5/f7）。`fail_pr_label(pr, count)` /
  `fail_label(issue, count)`（f6）/ `push_pr_comment_from_other(pr, body)`（f4）を追加。
...
- `escalation.rs` 単体（f2）: `fail_label(issue, 1)` で issue の `add_label` を1回失敗させ、
  `working` が外れ・comment/event/notify なし → 次呼び出しで成功しコメント1件。
- `escalation.rs` 単体（f5/f7）: `pr_comment` 投稿後に `comments_of` と `pr_comments_meta` の
  **両方**から同じコメントが見えることを確認（両ビュー記録の回帰）。
