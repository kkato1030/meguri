...
- 自己レビュー・フェーズが**レビュー機構を結論まで走り切った**とき、次の **終端イベントちょうど1つ**が出る
  ...
  - `self_review.needs_human` `{round}` — reviewer が verdict で人間判断を要求
  ただし機構自体が途中で人間へ抜ける経路は **終端イベントを出さない**:agent が review/fix turn で
...
- **phases**: 完走フェーズ数 = 終端イベント総数(clean + unconverged + needs_human)。機構が途中で人間へ
  抜けたフェーズ(agent failure・契約違反再失敗・中断)は終端イベントを出さないので入らない(決定1)
...
1. **分母は「完走したフェーズ」= 終端イベントの三つ組。** phases = count(clean)+count(unconverged)+
   count(needs_human)。これは cap 落ち率の分母を「レビュー機構が結論まで到達したフェーズ」に限る明示的な
   観測境界であって、self-review に入った run 総数ではない。機構自体が途中で人間へ抜ける経路(agent が
   `Failure`/`NeedsHuman`/`NeedsPlan`/`Decompose` を返す・2回目の契約違反・pane 死/中断)は終端イベントを
   出さず、分母に入らない。この計測は read-only なので、これらの経路に新イベントを足すことはしない
   (足すなら self_review.rs の挙動変更 = 別 issue)。「機構が完走せず人間に抜けた率」は別種の運用コストで、
   必要になったら後続スライスで別指標として足す(範囲外)。完走フェーズは run につき高々1回で、終端イベントは
   1つ(resume は `self_review_converged` で短絡し二重に出さない)ため distinct run_id を数える必要はない。
...
- 終端イベントを出さないフェーズ(中断、および agent failure・契約違反再失敗の人間抜け経路 —
  `self_review.correction` は出るが終端イベントは無いケースを含む)が分母に入らない。
- `--project` スコープが効く。
