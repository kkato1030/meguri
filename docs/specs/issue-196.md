...
- **slug の厳密検証**: `<owner/repo>` は `^[A-Za-z0-9._-]+/[A-Za-z0-9._-]+$`(owner と repo が
  ちょうど 1 つの `/` で区切られ、両者が非空、`.`/`..` 単独やパストラバーサル・空白・改行を含まない)
  を満たす時だけ受理する。GitHub の許容文字に寄せた保守的な集合。外れたら即拒否。
...
- **単体**: (1) ブロック整形、(2) id 導出 + `validate_project_id` 拒否、(3) 既存 config との
  ...
  引用符・改行・バックスラッシュ・複数 `/`・空 owner/repo・空白を含む入力を拒否すること、および
