...
- **`meguri init` の live stub 問題**: (A) init の `[[projects]]` stub をコメント化 / (B)
  add-project が未編集 stub を検出して置換。
  → **(A) を採る**。現在の `INIT_TEMPLATE`(`src/config.rs:45-47`)は `id = "myproj"` /
  `repo_slug = "owner/repo"` を**有効な** `[[projects]]` として書く。add-project は末尾追記なので、
  案内どおり `meguri init` → `meguri add-project owner/repo` を実行すると、実プロジェクトに加えて
  ダミーの `owner/repo` が残り、doctor/watch がそれを実 GitHub project 扱いして clone/権限検査が
  赤くなる(受け入れ基準 1 と矛盾)。そこで `INIT_TEMPLATE` の `[[projects]]` stub を他セクションと
  同じく**コメント化された例**にし、`meguri init` 直後の config が**有効プロジェクト 0 件**に
  パースされるようにする。追記される最初の実プロジェクトが唯一の live entry になる。
  (B) は「未編集」の判定が文字列一致に依存して脆く、append-only 設計に置換ロジックを持ち込む。
  (A) なら置換対象が存在せず、追記だけで筋が通る。
...
- **統合**: gh の repo 作成・clone は network/gh 依存で FakeForge の範囲外。orchestration core を
  ...
  clone 部分の確認に流用可能。
- **init テンプレート整合**: (7) `INIT_TEMPLATE` を `Config::parse` して `projects` が空
  (有効プロジェクト 0 件)であること、(8) `meguri init` 直後の config へ
  `add-project owner/repo` 相当の追記をして再 `load` → プロジェクトがちょうど 1 件
  (ダミー `owner/repo` が残らない)であることを検証する。
...
8. `meguri init` の `[[projects]]` stub がコメント化され、init 直後の config が有効プロジェクト
   0 件にパースされる。init の案内も「stub を手編集」から `meguri add-project` 誘導へ更新される。
   その結果 `meguri init` → `meguri add-project owner/repo` でダミー `owner/repo` が残らない
   (受け入れ基準 1 と整合する)。
...
- `src/config.rs` — config 末尾追記関数(原子的書き込み + 値の TOML serialize)、slug 検証、
  id 導出、`INIT_TEMPLATE` の `[[projects]]` stub をコメント化(有効プロジェクト 0 件)+ 誘導文言
