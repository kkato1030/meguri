...
3. **`src/store/runs.rs`** — 非 active だが待っている run を拾う専用クエリ `list_parked_awaiting_human()`(`interaction_state='awaiting_human' AND status NOT IN ('queued','running','interrupted')`)を追加。クリアは既存 `update_interaction_state(id, None)`(`:685-693`)を再利用。
