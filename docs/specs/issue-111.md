...
- `src/engine/flow.rs` — `Flavor::supports_advisor`、`ensure_advisor`(execute 前 spawn、捨てて張り直し、best-effort、成否を execute へ)、`release_advisor`(run 終端 reap + ディレクトリ削除)
- `src/engine/worker.rs` / `src/engine/spec_worker.rs` — `supports_advisor() = true`、execute プロンプトへの相談ブロック append(advisor spawn 成功時のみ)
- `src/engine/scheduler.rs` — 加重スロット会計(collab 有効の worker / spec-worker run = 2)
