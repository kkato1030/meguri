...
```sql
-- 各 run を統べた collab の面(#121 測定)。NULL = この列より前の run。
-- 'off' / 'advisor'。routing_arm(0014)と同じ後付け nullable の流儀。
...
- stamp する値は **`collab::run_gets_advisor(cfg, run)` が真なら `"advisor"`、偽なら `"off"`**。これは「この run に実際に advisor が付く編成だったか」を表す(local task や非対象ループは `"off"`)。spawn の best-effort 失敗(ADR 0006)とは独立に、**意図した面**を刻む — 比較したいのは編成であって spawn の当たり外れではない。
...
- `scored_outcomes` の projection に `COALESCE(collab_mode, 'off')` を1列足す(NULL の旧 run は `off` 扱い = 面が無かった=off として素直)。
- `collab_stats(project, window) -> Vec<CollabStatRow>`:key を `(project_id, loop_kind, collab_mode)` にして `WindowAgg`(既存)で集計。**advisor 対象ループ(worker / spec-worker)に絞る** — `collab::supports_advisor_loop_kind` を再利用し、面の比較が意味を持つ行だけ出す。
...
新しい面・新しい制御経路は増えない。measurement は**読み取り専用の派生ビュー**で、`stats routing`(#65, ADR 0002 の「serve は sqlite 直読み」)と同じく watch 停止中でも動く。唯一の書き込みは run launch 時の1列 stamp で、これは routing_arm の stamp と同じタイミング・同じ性質(意思決定に使わない観測用メタデータ)。完了契約・検証・scheduler には一切食い込まない(横断制約:協働面は完了契約を構造的に汚せない、を測定も守る)。
...
- **Migration:** `ALTER TABLE runs ADD COLUMN collab_mode TEXT`(0015)。既存 run は NULL、集計側で `off` に畳む。追加のみで破壊なし・前方互換(routing_arm 0014 と同一手口)。
- **Rollback:** SQLite は `DROP COLUMN` を避ける流儀(既存 migration 参照)。ロールバックは「列を無視する」= 旧バイナリは `collab_mode` を SELECT しないので害なし。`[collab]` 未指定なら stamp は常に `off` で、**現状とバイト単位同一**の inert 規律(横断制約)を破らない。データ損失経路なし。
...
1. `off` の worker run 群と `advisor` の worker run 群を seed → `collab_stats` が2行に割れ、それぞれ success_rate / avg_turns が正しい。
2. `collab_mode` NULL(旧 run)が `off` 行に畳まれる。
3. `planner` / local task など非対象ループは `collab_stats` に**出ない**(supports_advisor_loop_kind フィルタ)。
4. window が新しい N 件だけを見る(routing と同じ)。
5. `run_gets_advisor` が真/偽で stamp 値が `advisor`/`off` に分かれる(stamp 地点の単体 or 結合テスト)。

`cargo fmt --check` / `clippy -D warnings` / `nextest` / `--doc` を通す(CI 同順)。

## 受け入れ基準

1. `runs.collab_mode` 列が migration 0015 で足され、run launch 時に `run_gets_advisor` に基づき `advisor`/`off` が stamp される。
2. `meguri stats collab` が `(project, role, collab_mode)` 別に成功率・平均 turns・平均所要時間を表示し、watch 停止中でも動く(sqlite 直読み)。
3. 集計は worker / spec-worker に絞られ、非対象ループは出ない。旧 run(NULL)は `off` に畳まれる。
4. `[collab]` 未指定なら stamp は `off` 一択で、現状とバイト単位同一(inert 規律)を破らない。
