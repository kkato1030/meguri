...
ただし key を `collab_mode` だけにすると**物差しがずれる**。collab を有効にした時期に worker の profile や explore 比率も動いていたら、`advisor` 行の成績変化が collab 由来か model/routing 由来か切り分けられない。`stats routing` が `agent_profile` と `routing_arm`(explore/escalated を本線に混ぜないための列、#66)を key に入れているのと同じ理由で、**collab の効き目を測るには routing 軸を固定して比べる**必要がある。

- key = `(project_id, loop_kind, agent_profile, routing_arm, collab_mode)`。`routing_stats` の key(`agent_profile`, `routing_arm` まで)に **`collab_mode` を1軸足しただけ**。同じ `(role, profile, arm)` の中で `off` 行と `advisor` 行が並び、routing を固定したまま collab だけを比較できる。profile や arm が collab と同時に動いたら、行は別グループに落ちて偽の帰属をしない。
- `collab_mode` は `off` / `advisor`。コマンド名は `meguri stats collab`(`stats routing` の隣。`ensemble` を名乗らない)。
- これでも観察データであって無作為化実験ではない(issue 難易度の分布など残る交絡はある)。だから ADR 0017 では「routing を固定した durable 信号の比較」と正直に位置づけ、因果を過剰主張しない。
...
3. **主戦場は実 GitHub issue。** #54 ローカルは step 3(別 issue)。v1 の測定対象は **GitHub issue-backed の run だけ**にする — local task は advisor 非対象(team 名が issue scope、ADR 0006)なので `collab_stats` から除外する(後段の `issue_number IS NOT NULL` フィルタ)。
...
- 「off を刻まず NULL のまま」にするのは inert 規律のため:`[collab]` 未指定なら `run_gets_advisor` は常に偽で、`collab_mode` への UPDATE は**一切発生しない**。migration で新列は増えるが(スキーマは変わる)、**既存 run の意味も既存列の値も変わらず、新列は全 run で NULL のまま**。制御挙動も現状どおり(集計は下記のとおり NULL を `off` と読む)。
...
- `scored_outcomes`(既に `agent_profile` と `routing_arm` を持つ)の projection に `COALESCE(collab_mode, 'off')` を1列足す(NULL = 書かれなかった run = 面が無かった=off として素直に読む)。
- `collab_stats(project, window) -> Vec<CollabStatRow>`:key を `(project_id, loop_kind, agent_profile, routing_arm, collab_mode)` にして `WindowAgg`(既存)で集計。**routing の key に `collab_mode` を1軸足しただけ**なので、`routing_stats` のバケット処理をそのまま流用できる(arm 分割ロジックも活きる — 別軸を殺さない)。
- ただし対象を**「advisor が付きうる run」だけに絞る** — `run_gets_advisor` の3条件のうち load-bearing な2つ:(a) advisor 対象ループ(`collab::supports_advisor_loop_kind` = worker / spec-worker)かつ (b) **GitHub issue backed**(`issue_number IS NOT NULL`)。local task の worker/spec-worker は advisor を受け取れない(team 名が issue scope、ADR 0006)ので、`off` 基準を汚さないよう集計から外す。この絞り込みは `scored_outcomes` に条件を足すのではなく `collab_stats` 側で行う(routing 集計は絞らないため)。
- 再利用は `WindowAgg` / `scored_outcomes` / バケット処理に留め、共通化しすぎない。

### 4. CLI:`meguri stats collab` — `src/cli.rs`(`StatsCommand` に `Collab`)+ `src/app.rs`(`cmd_stats_collab`)

`cmd_stats_routing` と同じ表描画。列は `PROJECT / ROLE / PROFILE / ARM / COLLAB / RUNS / SUCCESS / AVGTURNS / AVGDUR`(routing の列に `COLLAB` を挟んだ形)。行が `(role, profile, arm)` でソートされ、同じ routing の `off`/`advisor` が隣り合って読める。行が無ければ `no collab stats yet`。
...
- **key を `collab_mode` だけにする(routing 軸を落とす)** → collab の効果と profile/routing の差が混ざり物差しがずれる(pr-review 指摘)。routing を key に残し、固定した上で collab だけを比べる。
...
1. 同じ `(worker, profile-A, main)` で未 stamp(NULL)の run 群と `advisor` の run 群を seed → `collab_stats` が `off`/`advisor` の2行に割れ、それぞれ success_rate / avg_turns が正しい。
2. `collab_mode` NULL(未 stamp / 旧 run)が `off` 行に読まれる。
3. **profile または arm が違う run は別行に分かれる**(collab 以外の軸が混ざらない)— 例:同じ collab_mode でも profile-A と profile-B は2行、main と escalated は2行。
4. `planner` など非対象ループ、および **local task の worker/spec-worker(`issue_number` NULL)は `collab_stats` に**出ない**(loop-kind フィルタ + issue-backed フィルタの両方)。
5. window が新しい N 件だけを見る(routing と同じ)。
6. `run_gets_advisor` が真なら `"advisor"` が stamp され、偽なら列は NULL のまま(書き込みが起きない)ことを確認(stamp 地点の単体 or 結合テスト)。
...
2. `meguri stats collab` が `(project, role, agent_profile, routing_arm, collab_mode)` 別に成功率・平均 turns・平均所要時間を表示し、watch 停止中でも動く(sqlite 直読み)。同じ `(role, profile, arm)` の `off`/`advisor` が隣り合い、routing を固定して collab だけを比較できる。
