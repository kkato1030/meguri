...
- **claim 時に一度だけ読み、run の `Checkpoint` に pin する。** 以後の検証・PR 作成・prompt は pin 値を
...
**pin の設定は `prepare_worktree` 系(`create_worktree` / `attach_worktree` 直後)で一度だけ**:
worktree ができた直後、`cp.repo_config` が未設定なら `RepoConfig::load_from_worktree(&wt)` を呼び、
結果(または `Err` 時は `default()`)を `cp.repo_config` に入れて checkpoint を persist する。既に
設定済み(resume)なら**読まない**。`base_sha` を conflict-resolver run で claim 時に固定するのと同じ配置。

**run スコープの effective `Deps` に畳み込む** — `drive`(`src/engine/flow.rs:370`)の頭側で、pin 済み
`cp.repo_config` を `deps` に畳み込んだ effective clone を作り、以降の step はそれを使う:

```rust
// host project が明示していないフィールドだけ repo pin 値で埋める。
let deps_owned;
let deps = if let Some(repo) = cp.repo_config.as_ref().filter(|_| /* 何か repo 値がある */) {
...
この結果、`pr_for` / `clean_for` / `language_for` や `deps.project.check_command`(validate,
