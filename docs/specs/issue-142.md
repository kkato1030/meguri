...
4. **振動防止**: (a) 比較前に本文を正規化(前後空白の trim + 連続空白の畳み込み + 改行の LF 統一)して whitespace-only / reflow の微小編集を無視する。(b) 再処理が succeeded したら新しいダイジェストを記録するので、同一内容の再ポーリングで再発火しない。(c) **`issue.body_changed` イベント・シグナルコメントとも「signaled 済みダイジェスト」で dedup する**(半身 A/B 共通、下記)。discover / sweep は毎 tick 走り events は append-only なので、dedup が無いと本文が変わった後 run がすぐ成功しない間(blockers / hold / slot 不足)にイベントが poll ごとに増える。よってイベント発火も digest 単位で一度きりに絞る。編集距離ベースの typo 閾値は過剰なのでスコープ外(正規化 + label gate + `implementing` では `ready` が無いので自動起動しない、で十分バウンドされる)。
...
- **記録**: **`claim_task` ではなく `drive` の共通ステップ**(`src/engine/flow.rs:308` の `prepare_work` が `Claimed` を返した直後)で `checkpoint.issue_body` から正規化ダイジェストを計算し、run に保存(`runs.body_digest`)。理由: default flavor は `claim_task`(`flow.rs:506`)で `cp.issue_body` を埋めるが、**spec-worker は `SpecWorkerFlavor::prepare_work`(`src/engine/spec_worker.rs:177`)で `claim_task` を通らずに直接 `cp.issue_body` を埋める**。`claim_task` だけで記録すると spec-worker の succeeded run は永久に NULL ダイジェストになり、NULL 互換ルール(下記)で「常に一致」= 本文編集後も恒久抑止され、半身 A の guard 差し替えが spec-worker 側で効かない。両パスが `checkpoint.issue_body` を確定させた後の共通地点で記録することで、全 flavor の succeeded run に digest が残る。
- **判定**: `LabelTaskSource::discover`(`src/tasks.rs:180`)と `SpecWorkerLoop::discover`(`src/engine/spec_worker.rs:79`)の `issue_has_succeeded_run(...)` 呼び出しを `store.issue_processed_current_body(project, loop, issue, current_digest)` に置換。抑止を解除する(= 本文が変わっていた)と判明した **かつ その digest をまだ signal していない** ときだけ `issue.body_changed` イベントを emit し、`issue_reconcile` に signaled digest を upsert する(受け入れ基準 3 の traceability + 基準 2 の dedup)。既に signal 済みの digest なら emit しない(discover は毎 tick 走り、run がすぐ成功しない間もこのパスを通るため)。
...
- フェーズラベル(特に `implementing`)を持ち succeeded run がある issue について、現在の正規化ダイジェスト ≠ 直近 succeeded run の `body_digest` **かつ** `issue_reconcile` の signaled digest ≠ 現在のダイジェスト(= この新本文をまだ signal していない)なら:
  - `issue.body_changed` を emit、
  - 一度きりのシグナルコメントを投稿(「本文が更新されました。再実装が必要なら `meguri:ready` を付け直してください」)、
  - `issue_reconcile` に現在のダイジェストを signaled として upsert。
- **dedup**: 小さな `issue_reconcile(project_id, issue_number, signaled_digest, signaled_at)` テーブルに signaled 済みダイジェストを upsert する。イベント発火・コメント投稿とも upsert 前のこの比較で gate するので、**イベントもコメントも同じ新本文に対して一度きり**(振動防止 c)。half A の discover guard もこの同じテーブル・同じ比較を使う(sweep は `implementing`、discover は `ready` と対象ラベルが排他なので二重発火しない)。
...
- `src/store/mod.rs`(または新 `src/store/reconcile.rs`)— `issue_reconcile` の upsert / read(signaled digest の dedup、半身 A・B 共用)。
- `src/tasks.rs` — 本文正規化ダイジェスト helper + `LabelTaskSource::discover` の guard 差し替え + signal dedup。
- `src/engine/flow.rs` — `drive` の `prepare_work` 直後(全 flavor 共通)で `checkpoint.issue_body` から `body_digest` を記録。`claim_task` ではないので spec-worker の custom prepare path もカバーされる。
- `src/engine/spec_worker.rs` — discover の guard 差し替え(記録は共通 `drive` 側なので `prepare_work` の変更は不要)。
...
2. **振動しない。** 同一内容の再ポーリングで再発火しない(処理後に新ダイジェストを記録);whitespace-only 編集は正規化で無視;**`issue.body_changed` イベントもシグナルコメントも新本文ごとに一度きり**(`issue_reconcile` の signaled digest で gate、半身 A・B 共通)。discover / sweep が毎 tick 走っても、run がすぐ成功しない間にイベントが積み増しされない。
...
- `tasks` 単体: ラベル churn でダイジェスト不変 → 抑止継続、本文変更 → 抑止解除 + `issue.body_changed` emit。**同一本文で discover を複数 tick 回しても `issue.body_changed` は一度きり**(signal dedup)。正規化(whitespace-only 無視)。NULL ダイジェスト(レガシー run)は従来どおり抑止。
- **spec-worker 経路**: `SpecWorkerFlavor::prepare_work` を通った succeeded run にも `body_digest` が記録され(共通 `drive` 側で記録)、本文編集後に `SpecWorkerLoop::discover` の抑止が解除される。記録漏れ(NULL 化)で恒久抑止が残らないことを回帰テストで固定。
- `reconcile` sweep 単体(FakeForge): `implementing` issue の本文を変更 → イベント + シグナルコメント一度きり、再 sweep で追加イベント・コメント無し。`signal_comment = false` でコメント無し・イベントのみ。`body_edits = false` で完全無効。
