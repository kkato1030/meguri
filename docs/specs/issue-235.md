...
これで prime は yolo（`--dangerously-skip-permissions`）を纏う必要が消える。claude の既定 prime は
`claude -p '<no-op>'`（yolo なし・permission-mode override なし＝既定の ask モード）を worktree の cwd で
一度走らせるだけ。headless `-p` は唯一ゲートを素通りする経路で、その一回で当該パスのフォルダ信頼を
CLI 自身が書き残す。フィールド名を meguri が知る必要はない（version-stable）。代替（JSON 直書き /
meguri 所有 config-dir / launch mode 変更）を退けた理由は ADR 0027。

**なぜ yolo を外すのが決定的か（f10/f12/f13 の根治）**: prime は worktree 上で実モデルターンを走らせ
repo の CLAUDE.md を読むので、yolo だと外部 PR 等の prompt injection が pane 起動前に任意のツール操作を
誘発しうる。yolo を外し ask モードにすると、headless には権限プロンプトに答える人間が居らず、どのツール
呼び出しも承認されない — **injection が指示を奪ってもアクチュエータが無い**。脆いツール封じフラグに
頼らず、「headless × 非 yolo ⇒ ツール実行不可」という堅い性質だけで安全を担保する（D2/f12）。

**empirical 検証（実装の最初のステップ、この判断の前提）**: 実機で `claude -p '<no-op>'` を worktree の
cwd で一度走らせた後、(a) 同 cwd への yolo 対話起動がフォルダ信頼プロンプトに当たらないこと、(b) 敵対的な
CLAUDE.md を置いても prime がツールを一切実行しないこと、を確認する。確認できれば D1 のまま。万一
非 yolo `-p` がフォルダ信頼を永続化しないと判明したら ADR 0027 の rejected 案 2（meguri 所有 config-dir、
資格情報の供給・保護・分離・削除を別 issue で設計）へ切り替える — 分岐点と条件はここで確定済み。

### D2. config スキーマ = `AgentProfile.preflight: Option<Vec<String>>`（既定は固定の安全 argv）
...
- 省略 → known-CLI の既定: `claude` → **`["-p", PREFLIGHT_NOOP_PROMPT]`（固定・yolo なし・permission-mode
  override なし）** ／ それ以外（cursor-agent 含む）→ `[]`（cursor-agent は `--trust`/`--force` を
  `args` で毎回素通りするので prime 不要）。

**f12 の決定 — 既定 argv を一つに固定する（`profile.args` を鏡写さない）。** 前ラウンドの「args を鏡写す」
案は yolo を prime に持ち込み injection 面を生むため撤回する（D1）。prime の担当はフォルダ信頼だけで、
それは permission mode に依存しないので、既定 argv は profile に関わらず **ちょうど `["-p",
PREFLIGHT_NOOP_PROMPT]`**。yolo フラグも permission-mode フラグも付けない。これにより:

- 安全性は脆いツール封じフラグ（CLI・バージョンで揺れ、`--dangerously-skip-permissions` が allowlist を
  無視する恐れもある）に依存せず、「headless × 非 yolo ⇒ ツール承認者が居ない ⇒ ツール実行不可」という
  堅い性質で担保される。受け入れ基準は「解決後の既定 argv が `["-p", PREFLIGHT_NOOP_PROMPT]` に一致し
  `--dangerously-skip-permissions` を含まない」ことを厳密に検査できる。
- **実機検証に失敗した場合の扱い**: 非 yolo `-p` がフォルダ信頼を書かないと判明したら、prime を諦めて
  meguri 所有 config-dir 案（ADR 0027 rejected 案 2）へ切り替える（別 issue）。yolo を prime に足して
  誤魔化す道は選ばない — それは f10 の injection 面を復活させるため。

`PREFLIGHT_NOOP_PROMPT` は何も変更させない固定文字列（例: `"reply ok and make no changes"`）。prime は
実モデルターンを1回・worktree cwd で・repo の指示（CLAUDE.md 等）を読んで走らせる副作用を持つ（トークン
消費と1往復）が、非 yolo・ツール承認者なしなので injection が指示を奪ってもツールは実行されない。
この副作用と安全性の根拠は ADR 0027 と README に明記する。

**f13 の決定 — 明示 `preflight` override は「安全策のない危険な opt-in」として明示し、警告する。** 既定は
上のとおり安全だが、host config が非空 `preflight` を明示すると argv はそのまま実行され、既定の非 yolo 縛りを
迂回できる。例えば `preflight = ["--dangerously-skip-permissions", "-p", "ok"]` と書けば、外部 PR の
CLAUDE.md を読んだ実モデルが pane 起動前に Bash/Edit 等を承認付きで動かせる。これを黙って許さない:
...
- `src/config.rs` — `AgentProfile` に `preflight: Option<Vec<String>>`（`#[serde(default)]`）を追加。
  既定解決は `routing`（`effective_headless_args` の隣）に `effective_preflight_args(command)` として
  置く。既定は `claude` → 固定 `["-p", PREFLIGHT_NOOP_PROMPT]`、他 → `[]`（D2）。加えて明示 override が
  yolo フラグを含む場合の起動時警告（f13、`launch::validate` 同様の一括チェック）。
...
- `src/preflight.rs` 単体: `effective_preflight_args` の解決 — **claude → ちょうど `["-p",
  PREFLIGHT_NOOP_PROMPT]`（`--dangerously-skip-permissions` を含まない）**／`[]` opt-out／明示 argv は
  そのまま／unknown command → `[]`（f12）。実行体は seam を切って注入 — spawn 成功/非ゼロ/timeout の
  3分岐を実プロセス無しで検証。
...
4. timeout 超過時、子プロセスは process-group ごと回収され、prime が `spawn_agent_pane` を塞ぐのは
   ...
   進む（f4、async 非ブロッキング）。
5. `preflight` 省略時、claude は **ちょうど `["-p", PREFLIGHT_NOOP_PROMPT]`**（`--dangerously-skip-
   permissions` を含まない）に解決され、フォルダ信頼だけを書く（bypass は書かない）。cursor-agent 等は
   空（f12）。明示 `preflight` に yolo フラグを含めると config ロード時に警告が出る（f13、ブロックはしない）。
