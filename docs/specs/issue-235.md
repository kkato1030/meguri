...
**安全担保の方式を1つに確定する（plan review 3 f1）**: これまで候補を並べていたが、実装できる形に
1つへ絞る。まず、退けた緩和策（理由も固定）:

- 「非 yolo だから安全」は誤り — 継承 `settings.json` の allow／`defaultMode` で CLAUDE.md 経由ツールが動く。
- `--permission-mode plan` も不十分 — plan は Read と read-only Bash を許し、allow ルールはモードの上に乗る。
- `--allowedTools` は許可リストではなく **自動承認リスト** なので、空にしても deny にはならない。

**採用する方式 = meguri 所有の deny-all `--settings` ファイル ＋ MCP 完全遮断。** prime だけに、meguri が
書いた settings ファイルを `--settings <path>` で渡す。継承 config の allow より deny が勝つ（Claude Code の
permission は deny 優先）ので、permissive な config-dir でもツールは通らない。ファイル内容は meguri が完全に
握るので決定的で、単体テストの期待値も injection test の合否も書ける。具体は下記 [prime 仕様] に固定する。

**[prime 仕様]（この spec/ADR 内で確定）**:

- **deny-all settings ファイル**: `~/.meguri/preflight/deny.json`（meguri が起動時に冪等生成、パーミッション
  `0600`）。内容:
  ```json
  {
    "permissions": {
      "deny": ["Bash", "Read", "Edit", "Write", "Glob", "Grep", "WebFetch",
               "WebSearch", "Task", "NotebookEdit", "TodoWrite", "mcp__*"],
      "defaultMode": "plan"
    },
  ...
  ```
  deny は現行 built-in tool 名を網羅し、`mcp__*` で MCP tool も塞ぐ。`defaultMode: "plan"` は多層防御。
- **MCP 遮断**: prime argv に `--strict-mcp-config` を付け、`--mcp-config` を渡さない → MCP サーバを一つも
  ロードしない（project `.mcp.json` も無視）。settings 側の 2 キーと二重で塞ぐ。
- **prime argv（claude・対応版）**: `[<pane と同じ --model>（あれば）] ++ ["--strict-mcp-config",
  "--settings", "<~/.meguri/preflight/deny.json>", "-p", PREFLIGHT_NOOP_PROMPT]`。yolo は付けない。
- **CLI 互換**: この方式は `--settings`／`--strict-mcp-config`／`permissions.deny` を持つ claude を要する。
  injection test が通る最小版を実装時に確定して定数 `PREFLIGHT_MIN_CLAUDE_VERSION` に固定し、doctor の
  version 情報（`routing::probe`）で判定する。**それ未満・版不明・非 claude では prime を skip**（下記）。

**この PR 内で使うフォールバック（別 issue に投げない）**: prime を skip したときは pane をそのまま起動する
（D5 と同じ best-effort）。つまり最悪でも「今日と同じ folder-trust gate で止まって needs-human」に戻るだけで、
**安全でない prime は決して走らせない**。config-dir を meguri 所有にする案（ADR 0027 rejected 案 2）は、この
skip フォールバックでも足りない場合の将来のハードニングとして残すが、本 PR の実装はこの spec 内で完結する。

**empirical 検証（実装の最初のステップ）**: 実機で上の prime argv を worktree の cwd で走らせ、(a) 同 cwd への
yolo 対話起動がフォルダ信頼プロンプトに当たらないこと、(b) **`settings.json` に `defaultMode = "acceptEdits"`
＋広い allow（Bash allow 含む）を置き MCP サーバも設定し、敵対的な CLAUDE.md（Read/Bash/MCP を各々促す）を
置いても、prime が built-in tool・Bash・MCP を一つも実行しないこと**、(c) この方式が通る最小 claude 版、を
確定する。(b) が通らない版は skip。これで deny リスト・MCP 遮断・版下限が数値/内容で固定される。
...
- 省略 → known-CLI の既定: `claude`（対応版）→ **D1 [prime 仕様] の argv**（`[<pane と同じ --model>]（あれば）
  ＋ `--strict-mcp-config --settings <deny.json> -p PREFLIGHT_NOOP_PROMPT`）／ 版が下限未満・不明、非 claude、
  cursor-agent は `[]`（prime skip。cursor-agent は `--trust`/`--force` を `args` で毎回素通りするので不要）。
...
**f1 の決定（plan review 2/3）— 実行封じは D1 [prime 仕様] で確定。** `--permission-mode plan` も
`--allowedTools ""` も不十分なので、meguri 所有の deny-all `--settings`（`permissions.deny` に全 built-in
tool ＋ `mcp__*`）＋ `--strict-mcp-config` で全ツール面を塞ぐ。deny リスト・MCP 遮断・argv・版下限・skip
フォールバックは D1 [prime 仕様] に固定済み。injection test（test strategy / 受け入れ基準5）が実効を証明する。

- **版が下限未満・実効不能のときの扱い（この PR 内で完結）**: (a) claude が `PREFLIGHT_MIN_CLAUDE_VERSION`
  未満／版不明、または (b) この方式が folder trust を書けない、と判明した版では **prime を skip** し pane を
  そのまま起動する（D5）。最悪でも今日と同じ folder-trust hang → needs-human に戻るだけで、安全でない prime は
  走らせない。meguri 所有 config-dir 案（ADR 0027 rejected 案 2）は skip でも足りない場合の将来ハードニングで、
  本 PR の実装はこの spec で完結する。yolo を prime に足して誤魔化す道は選ばない（injection 面を復活させる）。
...
- `src/routing.rs` — 既定解決 `effective_preflight_args(profile, command, claude_version)` を
  `effective_headless_args` の隣に置く。対応版 `claude` → `[<profile.args から抽出した --model>]（あれば）＋
  `["--strict-mcp-config", "--settings", <deny.json path>, "-p", PREFLIGHT_NOOP_PROMPT]`（D1 [prime 仕様]）、
  版が下限未満・不明／他 command → `[]`（skip）。`profile.args` から `--model`/`-m`（空白・`=`形）を抜く小関数を
  足す（`effective_headless_args` は model を保証しないので使わない）。加えて明示 override が yolo フラグを
  含む場合の起動時警告（f13、`launch::validate` 同様の一括チェック）。`PREFLIGHT_MIN_CLAUDE_VERSION` 定数もここ。
- `src/preflight.rs`（新規）— prime の async 実行体 + 「一度だけ」の統制。`run_preflight(command,
  ...
  async timeout + reap、D4）。加えて (i) deny-all settings ファイル `~/.meguri/preflight/deny.json` を
  0600 で冪等生成（D1 [prime 仕様] の内容）、(ii) キー `(command, config_dir, argv, 絶対対象パス)` の
  ensure 関数: `~/.meguri/preflight/<hash>` マーカー（f11、`config::meguri_home()` 配下）の確認、プロセス内
  async ロックでの直列化、claim-once の記録（f6/f7/f8）。
...
- `preflight.skipped`: `{ profile, command, reason: "unsupported_version"|"non_claude"|"opt_out" }`。
  版下限未満・非対応 CLI・`preflight = []` で prime を出さなかった経路（安全側 skip、失敗ではない）。
- `pane.spawned` イベントに `preflight` の要否/結果を1フィールド足すかは実装時判断（任意）。

## test strategy

- `src/preflight.rs` 単体: `effective_preflight_args` の解決 — **対応版 claude → 解決後 argv が (i) pane と
  同じ `--model`（`profile.args` 由来。builtin `--model opus` も custom `args=["--model","haiku"]`＋
  `headless_args` 省略も pane と一致、f2）を保ち、(ii) `--strict-mcp-config --settings <deny.json> -p NOOP`
  を含み、(iii) `--dangerously-skip-permissions` を含まない、を検証。版が下限未満・不明／非 claude → `[]`
  （skip）。`[]` opt-out／明示 argv はそのまま も（f3: 古い `["-p", NOOP]` 固定は廃止）。`deny.json` 生成が
  D1 [prime 仕様] の内容・`0600` であることも単体で確認。実行体は seam を切って注入 — spawn 成功/非ゼロ/timeout
  の3分岐を実プロセス無しで検証。
- **f1 の実行不能（安全ゲート・実機）**: `settings.json` に `defaultMode = "acceptEdits"`＋広い allow
  ...
  置いた cwd に、確定した prime argv（`--strict-mcp-config --settings <deny.json> -p NOOP`）で prime を走らせ、
  **built-in tool・Bash（read-only 含む）・MCP を一つも実行しない**ことを全面で検証する。合否条件は「実行された
  ツール数 = 0  かつ folder trust が記録された」。この test が最小対応版 `PREFLIGHT_MIN_CLAUDE_VERSION` を
  確定する。版が下限未満なら `effective_preflight_args` が `[]` を返し prime skip、という分岐も別テストで確認。
...
- **既定 argv（plan review 2/3）**: 対応版 claude の解決後 prime argv が、pane と同じ `--model` を保ち・
  `--strict-mcp-config --settings <deny.json> -p NOOP` を含み・`--dangerously-skip-permissions` を含まない
  こと。builtin と custom（`args` にだけ `--model`）の両方、明示 `headless_args` に yolo が混じっても既定
  prime に yolo が漏れないこと、版が下限未満だと `[]`（skip）に解決されることを検証。
...
4. timeout 超過時、子プロセスは process-group ごと回収され、prime が `spawn_agent_pane` を塞ぐのは
   ...
   進む（f4、async 非ブロッキング）。
5. `preflight` 省略時、対応版 claude は `[pane と同じ --model（あれば）] ＋ ["--strict-mcp-config",
   "--settings", <~/.meguri/preflight/deny.json>, "-p", PREFLIGHT_NOOP_PROMPT]` に解決され、
   `--dangerously-skip-permissions` を含まず、builtin/custom いずれでも pane と同一モデルを使う（f2）。
   `deny.json` は D1 [prime 仕様] の内容・`0600`。permissive な `settings.json`（`acceptEdits`＋allow＋MCP）と
   敵対的 CLAUDE.md の下でも prime は built-in tool・Bash・MCP を一つも実行しない（f1）。claude が
   `PREFLIGHT_MIN_CLAUDE_VERSION` 未満・版不明、非 claude、cursor-agent は `[]`（prime skip、pane はそのまま
   起動）。明示 `preflight` に yolo フラグを含めると config ロード時に警告が出る（f13、ブロックはしない）。
