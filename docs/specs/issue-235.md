...
- 省略 → known-CLI の既定: `claude` → **`profile.args` + ツール封じフラグ + `["-p", PREFLIGHT_NOOP_PROMPT]`**
  （ツール封じは f10 の緩和、後述）／ それ以外（cursor-agent 含む）→ `[]`（cursor-agent は
  `--trust`/`--force` を `args` で毎回素通りするので prime 不要）。
...
**f3 補足 — prompt injection（f10）: ツールを封じ、残余は明示的に受容する。** prime は yolo posture で
worktree 上の実モデルターンを走らせるので、repo の CLAUDE.md や外部 PR の内容に仕込まれた injection が
no-op 指示を上書きし、pane 起動前に任意のツール操作を誘発しうる。「no-op だから安全」で済ませず、こう扱う:

- **緩和**: 既定 prime の argv に「ツールを一切許可しない」制約を足す（`-p` の noop ターンで tool を
  実行させない）。bypass 受諾はモード（`--dangerously-skip-permissions`）に、folder trust は cwd に
  紐づき、どちらもツール実行を要さないので、ツールを封じても両 receipt は書ける。injection が指示を
  奪ってもアクチュエータが無い。D1 と同じく実機で「ツール封じ prime が両 receipt を書く」ことを検証し、
  壊れる/フラグが無ければ下の受容へ寄せる。
- **受容（残余）**: prime 直後に起動する実 pane ターンは、同じ worktree・同じ CLAUDE.md の上で
  full-yolo で走る。worktree の内容を信頼して yolo で動かす判断は ADR 0012（yolo 既定）で既に下され、
  外部内容が injection 面であることは ADR 0024 で既に明示済み。prime はこの信頼境界を新設せず、時刻を
  pane 起動前へ少し早めるだけで、ツール封じでその前倒し分も無効化する。ADR 0024 の方針（面を明示し
  既存の緩衝を防御と位置づける／実行時 sanitize は入れない）に揃える。判断と検証は ADR 0027 に記録。
...
- **identity 単位のマーカー（f6）**: 状態を worktree 単位の単一フラグにすると、同じ worktree で
  先に走った非 yolo / cursor-agent profile のマーカーが、後続の yolo claude profile に必要な prime を
  握り潰し、pane が元のゲートで停止する。そこでマーカーを **gate identity** 単位にする —
  `gate.rs` の `GateTarget` と同じ dedup キー `(command, 実効 config_dir, preflight argv)`。マーカー名は
  `.meguri/.preflight-<hash>`（hash はそのキーの短縮ハッシュ、git-excluded・worktree と一緒に消える）。
  yolo と非 yolo は preflight argv が違うので別マーカーになり、互いを skip しない。
- **直列化（f7）**: 確認→prime→書き込みが原子的でも直列でもないと、parallel self-review の複数
  reviewer が同じ worktree で同時に初回 spawn したとき全員がマーカー未作成を見て実モデル prime を
  重複実行する。プロセス内の非同期ロック（identity+worktree をキーにした
  `HashMap<Key, Arc<tokio::sync::Mutex<()>>>`）で critical section（マーカー確認→prime→記録）を
  直列化する。先着がロックを取って prime し、後着はロック解放後にマーカーを見て skip する（先着の
  prime 完了を待ってから自分の pane を起動するので、後着の pane はゲートに当たらない）。別 worktree・
  別 identity は別キーなので並行して走れる。複数 daemon プロセスの同時起動は通常構成外としスコープ外。
- **claim-once（f8）**: マーカーを成功時だけ書くと、spawn 失敗・非ゼロ・timeout の後は次の spawn
  ...
  マーカーに記録し、identity+worktree ごとに二度は試みない。`success` なら skip。`failed` でも自動
  ...
  これで D3/受け入れ基準1の「worktree（identity）ごと一度だけ」が数の上でも保証される。

**f9 の決定 — advisor pane も同じ machinery で prime する。** `spawn_advisor_pane`（`flow.rs`）も
fresh な `advisor-{issue}` ディレクトリで同じ対話 CLI を `profile.args`（既定 yolo）で直接起動するので、
初回 folder-trust / bypass ゲートが残り advisor が無人で停止する。よって advisor の pane spawn 直前にも
同じ prime（identity マーカー・直列化・claim-once・失敗フォールバック）を入れる。cwd は advisor_dir。
advisor dir は repo を持たない空ディレクトリなので prime が読む指示は無く（f10 の面も生じない）、より安全。
...
- `src/preflight.rs`（新規）— prime の async 実行体 + 「一度だけ」の統制。`run_preflight(command,
  ...
  async timeout + reap、D4）。加えて gate identity `(command, config_dir, argv)` + worktree をキーに
  した ensure 関数: identity 別マーカー `.meguri/.preflight-<hash>` の確認、プロセス内 async ロックでの
  直列化、claim-once の記録（f6/f7/f8）。
...
- `src/engine/flow.rs` — `spawn_agent_pane` と `spawn_advisor_pane`（f9）の両方に (a)
  ...
  pane spawn 直前に `Pane` mode のとき identity 単位の ensure 関数を呼ぶ（f2/f6/f7/f8）、(c) 結果を
...
- **f6 の identity 分離**: 同じ worktree で非 yolo（または cursor-agent）profile が先に prime／記録
  した後、yolo claude profile の初回 spawn が **なお prime を走らせる**こと（別マーカー）。
- **f7 の直列化**: 同じ worktree・同じ identity で複数の pane を同時に初回 spawn すると prime が
  ちょうど1回だけ走ること（後着はロック後に skip）。
- **f8 の claim-once**: prime を失敗させた後、同じ worktree・identity の次の spawn が prime を
  再実行しないこと（`failed` マーカーで打ち切り）。
- **f9 の advisor**: `spawn_advisor_pane` が pane spawn の前に advisor_dir で prime を1回走らせること。
- **f10 のツール封じ**: 既定 prime argv にツール封じフラグが載ること（引数レベルの検証）。実機検証項目
  として「ツール封じ prime が bypass 受諾＋folder trust を書く」ことを別途確認する（D2 の実機検証と束ねる）。
- 統合（既存の `tests/fixtures/fake_agent.sh` 系）: prime を実行してから pane 起動 → 完了
  コントラクトが返るまでを、prime をスクリプト化した fake CLI で通す（実 claude は叩かない）。

## 受け入れ基準

1. 新規 worktree の `Pane` role 起動で、pane spawn の前に prime が worktree cwd で **gate identity
   ごと一度だけ** 走り、その `CLAUDE_CONFIG_DIR` は pane の `PaneSpec.env` に載る絶対パスと一致する
   （f1）。resume 失敗→plain 再試行でも二重に走らない（f2）。同 worktree で先に別 posture の profile が
   走っても yolo profile は別マーカーで prime される（f6）。同時初回 spawn でも prime はちょうど1回
   （f7、直列化）。prime 失敗後は同 identity で再試行しない（f8、claim-once）。
...
7. README（en/ja）に pre-flight prime・実モデルターンの副作用・`preflight = []` 無効化・非 yolo profile
   では bypass を書かないこと・prompt injection 面（f10、ツール封じと残余の受容）が記述される。
8. 既存テストが全部通る。
