...
- **Rollback**: 列は追加のみで破壊的変更なし。2 つの仕組みは独立に config で止められる:
  - **explore を止める**: `explore_ratio = 0`(既定)。explore 経路は inert になり、
    割り当て軸は routing 1/3 と一致。
  - **エスカレーションを止める**: `[routing.escalation]` に `enabled = false` を明示する。
    エスカレーションは既定 `enabled = true` で、`worker`/`fixer` には既定チェーン
    (sonnet→opus)があるため、**`[routing.escalation]` を未設定にしただけでは検証失敗時の
    昇格は動く**(routing 1/3 とは一致しない)。旧挙動へ完全に戻すには `enabled = false` が
    必要。
  - 両方を止めれば(`explore_ratio = 0` + `enabled = false`)、挙動は routing 1/3 と
    バイト単位で一致する。
...
- **e2e(FakeMux、受け入れ条件そのまま)**:
  ...
  - **昇格の rollback**: `[routing.escalation] enabled = false` で検証失敗しても昇格せず、
    routing 1/3 と同一挙動になること。
