...
- **起点(チェーン内の位置で決める)**: 昇格するかどうか・どこへ上がるかは、**現在 pin
  されているプロファイル(`runs.agent_profile`)が `escalation_chain(role)` のどこに
  いるか**で決める。チェーン内にあり末尾でなければ次の entry へ 1 つ進む
  (`cp.escalation_level` は昇格回数の記録で、チェーン添字そのものではない)。
  **チェーンにいないプロファイルからは昇格しない**。routing 1/3 の契約
  (`src/routing.rs` の `resolve`: 明示はそのまま、manual と検出フォールバックは
  `default`)との対応は:
  - manual mode / auto フォールバックで `default` が pin された run — `default` は
    エスカレーションチェーンに現れない(validate で禁止、下記)ので昇格対象外。
    「manual/検出失敗は `default`」を黙ってすり替えない。
  - 明示 `[routing.roles]` で**チェーン外**のプロファイル(例: `worker = "codex"`)を
    pin した run — 昇格しない(明示は勝つ)。
  - 明示 pin が**チェーン内**(例: `worker = "claude-sonnet"`)の run — 昇格する。
    これは黙ったすり替えではない: `run.escalated` イベント + プロンプト明記で loud で
    あり、止めたければ `[escalation] enabled = false` か役割チェーンの上書き
    (`worker = []` で空チェーン)という独立のノブが 2 つある。
- **候補の検証**: config で上書きしたチェーンは起動時に `routing::validate` と同じ loud な
  面で検査する(未知の役割名、プロファイル名の存在、`default` の禁止)。昇格時は次 entry を
  存在(`profile_exists`)+ CLI 検出で確認し、使えなければ**チェーンのさらに上位**を試す
  (auto 解決 `resolve` が候補を読み飛ばすのと同じ流儀。ただし進む向きは強い方のみで、
  チェーン外へは出ない)。使える entry がなければ昇格せずその場に留まり、従来どおり
  `validate_turns` 超過で needs-human へ。
...
routing 1/3 以前の挙動をバイト単位で保つ(explore は `explore_ratio` が `[routing]` 内の
キーである時点で legacy では構造的に書けない——ゲートが効くのは `[escalation]` 側)。
チェーンは推奨表に由来する概念なので、routing が off の場所で昇格が走るのは筋が通らない。
この共通ゲートが後述の設定配置と rollback の土台になる。
...
- `src/routing.rs` — `escalation_chain(role) -> Vec<String>`(既定表 + config 上書きの
  ...
  `is_explore(issue_number, ratio) -> bool`(決定的ハッシュ)を追加。既存 `validate` を
  拡張し、config 上書きチェーンの未知役割名・プロファイル存在・`default` 禁止を起動時に
  loud に検査する。`recommended_chain` はそのまま(検出フォールバック用途、別物)。
- `src/engine/flow.rs` —
...
- `src/store/runs.rs` + `src/store/migrations/0011_routing_arm.sql` —
  ...
  番号は現在の末尾 `0010_schedules`(`src/store/mod.rs`)の次で、ADR 番号(0012)とは
  別系列。他 branch と衝突したら、このリポジトリの慣行どおり実装時に付け直す。
...
1. **エスカレーションチェーンは `recommended_chain` と別表**。前者は強くなる向き、後者は
   ...
   reviewer 系はチェーンなし。昇格の起点は現在 pin のチェーン内位置で決め、**チェーン外
   (`default` 含む)からは昇格しない**(本文 A の「起点」)。
...
- **Migration**: `0011_routing_arm.sql` は `ADD COLUMN routing_arm TEXT`(NULL 許容)。
...
- **単体(routing.rs)**: `escalation_chain` の既定と config 上書きマージ、昇格起点の判定
  (チェーン外プロファイル・`default`・チェーン末尾は昇格しない、チェーン内は次 entry、
  次 entry が存在/検出しなければ上位へ読み飛ばし)、`validate` のチェーン検査(未知役割・
  存在しないプロファイル・`default` 記載でエラー)、`is_explore` の決定性(同じ issue
  番号は常に同じ判定、ratio=0 で常に false、ratio=1 で常に true)、`explore_alternative`
  の次候補選択と no-op ケース。
...
- **e2e(FakeMux、受け入れ条件そのまま)**:
  ...
  - legacy 回帰: `[routing]` なし + トップレベル `[escalation]` のみ記載(`enabled = true`
    + 役割チェーン)→ 昇格が起きず、全 role が `default` の routing 1/3 以前と同一 spawn。
    なお `explore_ratio` は `[routing]` 内のキーなので「`[routing]` なしで `explore_ratio`
    あり」という設定は TOML 構造上作れない——legacy での explore の inert は構造的に保証
    され、explore 側の回帰は上の `[routing] explore_ratio = 0` ケースで担保する。
