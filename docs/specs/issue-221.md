...
pub struct PrObservation {
    ...
    pub current_head_armed: bool,        // 現在の head_sha に対する arm marker があるか（arm 冪等・再 arm の鍵）
    pub armed_since_any_head: Option<String>, // 任意 head の最古 arm marker の createdAt（stale 判定・「そもそも arm 済みか」）
...
**arm marker は head-keyed / head-independent の 2 面を分けて持つ**（既存コードでも別関数）。
`current_head_armed` は現在 head の marker 有無（`auto_merger::head_already_armed` 相当）で、arm の
冪等性と再 arm を鍵る。`armed_since_any_head` は任意 head の最古 marker の createdAt
（`merge_watch::armed_since` / `is_armed` 相当）で、Stuck の stale 判定と「そもそも arm 済みか」に
使う。**この 2 つを 1 フィールドに畳んではならない**：畳むと update-branch で head が進んでも古い
marker が残る限り「arm 済み」に見え、決定 4 の再 arm（未 arm と判定させる）が起きない。
...
merge-tail 状態空間と所有 arm（抜粋・優先順）。表中の「arm 済み」＝`current_head_armed == true`、
「未 arm」＝`current_head_armed == false`（＝現在 head に marker が無い）。「stale」は
`armed_since_any_head` から算出する：
...
（`armed_marker(head_sha)`）なので、head が動くと現在 head の marker が外れ、観測の
`current_head_armed` が false になる。次の観測で「未 arm × 適格」と判定されて自然に再 arm される。
つまり「再 arm」は明示の第 2 ステップではなく、level-triggered な観測から**創発**する（ADR 0012
§4「arm 1本で閉じる」の実体）。ここで判定に使うのは `current_head_armed`（現在 head）であって
`armed_since_any_head`（任意 head の最古 marker）**ではない**点が肝 — 後者で判定すると古い marker が
残る限り再 arm しない。エッジ駆動の「update したら arm しろ」という状態遷移をコードに書かない。
...
- **BEHIND 回帰テスト**（受け入れ 1）：FakeForge で arm→base 進行→`Op(UpdateBranch)`→head 更新→
  次観測で再 arm、を native / orchestrator 両モードで通す。**古い head の arm marker を残したまま**
  head を進め、`current_head_armed` が false に転じて再 arm が起きること（`armed_since_any_head` が
  Some のままでも再 arm が阻害されないこと）を明示的にアサートする。
