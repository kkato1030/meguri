...
- **内容取得は raw stdout helper を新設して使う(既存 `run_git` に乗せない)**。`run_git`
  (`src/gitops.rs:8`)は stdout を `from_utf8_lossy(...).trim_end()` して返すので、`cat-file blob` に
  流用すると `body_file` の**末尾改行・末尾空白が落ち、invalid UTF-8 も黙って `�` に置換**される。これは
  現行の `std::fs::read_to_string` と挙動が変わり、schedule が起票する本文を静かに書き換える。よって
  `cat-file blob` 専用に **trim せず・UTF-8 は明示エラーにする**バイト列取得 helper
  (例: stdout を `Vec<u8>` で受け、`String::from_utf8(bytes)` で厳密変換し失敗は `Err`)を
  `src/gitops.rs` に足し、`Content` はこの helper の結果を持つ。既存 `run_git` の trim 挙動は他の呼び出し
  (sha・path 取得等)が依存するので変えない。
...
- 新 gitops 関数: (a) origin 優先で default branch の通常 blob を `Content` で読む、(b) remote 無しで
  ...
  (i) base ref が無い→`Err`、(j) **末尾改行・末尾空白がそのまま保たれる**(`run_git` の trim で落ちない)、
  (k) **invalid UTF-8 を含む blob は lossy 置換せず `Err`**(現行 `read_to_string` と同じ厳密さ)。
