...
- `src/engine/mod.rs` に共通ヘルパー `canonical_issue(pr: &PullRequest) -> Option<i64>` を置き、各 loop の `discover` が `Target { issue_number: canonical_issue(&pr)…, … }` を返す。
- run が issue 番号鍵になるので、`prepare_work` は PR を再解決する必要がある。`open_pr_for_issue(deps, issue) -> Result<Option<PullRequest>>`(`list_open_prs` + `issue_from_branch` 照合; spec_worker の `spec_ready_pr` と同型)を共通化する。同一 issue に複数の open meguri PR が見つかったら skip(benign race 扱い、escalate しない)。
