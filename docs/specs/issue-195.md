...
/// 冪等: dest が既に有効な git repo なら no-op。
pub async fn ensure_bare_clone(dest: &Path, repo_slug: &str) -> Result<()> {
    if dest.join("HEAD").exists() { return Ok(()); }        // 既にある(bare は HEAD が直下)
...
- 失敗は `bail!` で loud に返す(認証・ネットワーク・slug 誤記)。
...
- **gitops 単体**: `ensure_bare_clone` をローカル bare origin(統合テストが既に使う土台)に対して
  ...
  (3) 2回目呼び出しが no-op、を検証。`gh` に依存しない経路(ローカルパス clone)でテストできる
  形にするか、gh 依存部を薄く分けてローカル origin で叩けるようにする。
- **config 単体**: `repo_path_for` の導出(省略時 `~/.meguri/repos/<id>`、明示時そのまま)、
  local mode + repo_path 省略の validate 拒否、github mode 省略の許容。
