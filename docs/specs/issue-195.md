...
- 準備に失敗した project は、その tick の redispatch・discover・sweep から**丸ごと除外**する
  (他 project は止めない)。`repo.clone.failed` を emit し、そのプロジェクト tick に
  `tracing::warn!` を1本出す。silent skip はしない。失敗の人間向け表示面は **doctor** に一本化
  する(下記「clone 失敗の通知先」で確定)。
- 成功時は `repo.cloned` を emit(下記 observability)。
- 一回きりの CLI(`meguri run` の one-shot)も同じ helper を prepare の前で呼ぶ。`meguri doctor`
  は例外で clone せず「未 clone」と報告するだけ(決定 5)。

### clone 失敗の通知先・冪等性(finding 対応)

**この失敗は run/issue/PR のどれも持たない。** tick 最上段の project 準備で起きるので、まだ
claim も run 生成もしていない。一方で既存の逃がし機構は3つとも**座標対象を必須**にする:
- `escalation.rs` の `escalate_task` / `escalate_issue` / `escalate_pr` は issue/task/PR 番号を
  取る(`src/engine/escalation.rs`)。
- `notify` の `Notification` は `run_id` と `issue_number` を必須にする(`src/notify/mod.rs`)。

よって clone 失敗を needs-human ラベルや `notify_awaiting_human` に**無理に載せない**。理由:
- **付ける先が無い。** needs-human は issue/PR に付くラベルだが、この段階でどちらも無い。偽の
  番号を発明するのは監査可能性(ADR)に反する。
- **性質が違う。** これは run が人の判断を待つ状態ではなく、**operator の config/認証/ネット
  ワークの問題**(slug 誤記・token の write scope 不足・オフライン)。座標対象に付ける needs-human
  ではなく、「設定は健全か」を見る面 = **doctor** が正しい宛先。
- **level-triggered で自己修復する。** 準備は毎 tick 冪等に再試行するので、operator が slug/認証/
  ネットワークを直せば次 tick で自動的に clone が通り、除外が解ける。恒久ラベルを貼る必要が無い。

**確定する宛先と冪等性**:
- **event**: 失敗 tick ごとに `repo.clone.failed`(slug / dest / 理由)を emit。level-triggered
  なので毎 tick 出る(dedup しない)—「まだ直っていない」の観測点であり、これが正。
- **ログ**: そのプロジェクト tick に `tracing::warn!` を1本(既存の sweep 失敗と同じ idiom、
  `scheduler.rs` の "sweep failed for {}" 群に倣う)。
- **人間向け表示**: **doctor** だけ。project ごとに「未 clone(これから reconcile)/ clone 失敗
  (残骸あり・理由)/ 健全」を区別表示する(決定 5)。doctor は現在状態を1回描くだけなので、
  スロットル対象になるスパム面は生まれない。
- **new-surface は作らない。** project-level の needs-human 面や notify 面をこの spec で新設する
  ことはしない(それは座標モデルを広げる durable な設計変更で、本 issue のスコープ外)。必要性が
  出たら別 issue + ADR で。
...
- `repo.cloned`(slug / dest)/ `repo.clone.failed`(slug / dest / 理由)を emit。後者は
  level-triggered なので失敗の続く限り毎 tick 出る(dedup しない)。
- clone 失敗は run/issue/PR を持たないので **needs-human ラベルや notify には載せない**
  (決定 3「clone 失敗の通知先」)。失敗 tick に `tracing::warn!` を1本(既存 sweep 失敗の idiom)。
- 人間向けの表示面は **doctor** に一本化。project ごとに「clone 済み / 未 clone(これから
  reconcile)/ clone 失敗(残骸・理由)」を区別表示し、gh auth write 権限の可否を出す。
...
- **config 単体**: `repo_path_for` の導出(省略時 `~/.meguri/repos/<id>`、明示時そのまま)、
  ...
  通ること。
- **clone hook**: repo_slug のみの Deps で tick を回し、未 clone → clone 実体化 →
  従来フロー(worktree→turn→PR)が FakeForge / scripted agent で通ること。**clone 失敗時**は、
  (1) その project がその tick の redispatch・discover・sweep から除外される (2)
  `repo.clone.failed` event が出る (3) needs-human ラベルや notify が**呼ばれない**
  (FakeForge にラベルが付かない・fake notifier が空)ことを検証する。失敗の人間向け表示は
  doctor 側テスト(下記)で担保する。
...
5. doctor が「未 clone(これから reconcile)」と「clone 失敗」を区別して表示し、gh auth の
   write 権限の可否を表示する。
6. clone 失敗(認証・ネットワーク・slug 誤記)は loud に surface する — `repo.clone.failed` event
   + tick の `tracing::warn!` + doctor 表示 — が、run/issue/PR が無いので needs-human ラベルや
   notify には載せない。silent skip はしない。壊れた `dest`(途中失敗の残骸)も次 tick で健全
   扱いせず loud に失敗する。
