...
| `src/engine/flow.rs` | `Checkpoint`: `self_review_ledger: Vec<LedgerEntry>` を追加(`self_review_pending` は**残して open エントリを mirror** — rollback 安全弁、下記移行節)。`self_review_last_head: Option<String>` と `self_review_final_fix_unreviewed: bool` を追加。`escalate_unconverged` 経路の見直し(cap→最終fix)。ループ冒頭短絡を converged/final-fix の両フラグ OR に。`compose_pr_body` / `self_review_details` / `post_self_review_status` を final-fix フラグで分岐(footer / commit status)。 |
...
loop:
  ...
  台帳の open が 0(=clean)       → 収束・publish(self_review_converged=true)
  rounds >= max_rounds(cap):
    残りは軽微な blocking のみ(ping-pong/decision 異議は上で捌け済み)
      → 最終 fix turn + validate → publish
        (self_review_final_fix_unreviewed=true だけ立てる。converged は立てない=rollback 安全弁。
         footer に「最終ラウンドの fix は未再レビュー」)
  それ以外:
  ...
  ※ 各 persist で open な ledger エントリを self_review_pending にも mirror(rollback 安全弁)
```
...
- `Checkpoint` に `self_review_final_fix_unreviewed: bool`(`#[serde(default)]`)を追加する。
  ...
  `self_review_converged` に依存しており、置換の blast radius を避けるため)。
- **2つのフラグの意味を分ける(rollback 安全のため意図的に非対称にする):**
  - `self_review_converged` は **clean 収束専用**のまま残す。旧バイナリ(#176)が知っている唯一の
    「publish してよい」印であり、意味を広げない。
  - `self_review_final_fix_unreviewed` は最終 fix→publish 経路**専用**の done 印。
- `final_fix_and_publish` は最終 fix + validate 後、**`self_review_final_fix_unreviewed = true` だけを
  立てて persist する(`self_review_converged` は false のまま)。** 新バイナリのループ冒頭短絡を
  `if cp.self_review_converged || cp.self_review_final_fix_unreviewed { Continue }` に広げ、この経路の
  resume も再レビューせず拾う。converged を立てない理由は下の rollback 節(旧バイナリが未再レビュー
  fix を `clean` として出すのを防ぐ安全弁)。
...
バイナリ更新をまたぐと問題になる。**前進(旧→新)と後退(新→旧・rollback)は非対称**なので分けて設計する。

### 前進(旧バイナリの checkpoint を新バイナリが読む)
...
- **移行方針:** `self_review_ledger` は `#[serde(default)]` で追加。`self_review_pending` は
  ...
  self-review 中だった run も台帳へ移行して継続でき、「resume でゼロからやり直し」を避ける。

### 後退(新バイナリの checkpoint を旧バイナリが読む = rollback)★安全弁の核心

`#[serde(default)]` は**新バイナリが古い JSON を読む方向**の互換でしかない。rollback 先の旧バイナリは
`self_review_ledger` も `self_review_final_fix_unreviewed` も**知らない**。素朴に revert すると
安全弁が2つ壊れる。両方を、旧バイナリが読める契約を新バイナリ側に持たせて塞ぐ:

- **(1) 未解決 finding を失わない — `self_review_pending` を mirror する。** 新バイナリは
  ledger を単独ソースにせず、**open な ledger エントリを毎 persist で `self_review_pending` にも
  写し込む**(この slice の必須契約)。rollback 後の旧バイナリは pending を読んで従来どおり
  fix→再レビューを続けられる。ledger だけに書いて pending を空にする実装は不可。mirror の撤去は、
  rollback 対象が #212 を含むバイナリに上がる**次 slice 以降**まで待つ。
- **(2) 未再レビュー fix を `clean` として出さない — done 印を分ける。** 上の escalation 節のとおり
  `final_fix_and_publish` は `self_review_converged` を**立てない**(`self_review_final_fix_unreviewed`
  だけ立てる)。この checkpoint(rounds==max_rounds・converged=false)を旧バイナリが resume すると、
  ループ冒頭で `rounds >= max_rounds` により **`escalate_unconverged`(#176 の挙動)= needs-human へ
  escalate** する。未再レビューの diff が `clean after N rounds` として公開されることは無く、最悪でも
  人間ゲートに載る(安全側に倒れる)。なお `final_fix_unreviewed` を立てるのは open-pr へ**戻る前**
  なので、この checkpoint 時点では PR 未作成(`pr_number` 未設定)。旧バイナリの escalate は
  needs-human 証拠 draft(ADR 0021)を1つ作るだけで二重にならない。既に open-pr まで済んで
  PR 作成後にクラッシュした run は step ポインタが self-review を抜けているので、旧バイナリは
  self-review に再入せず、この分岐に来ない。
- **追加フィールドはすべて `#[serde(default)]`:** `self_review_ledger` / `self_review_last_head` /
  `self_review_final_fix_unreviewed`。旧バイナリは未知キーを無視して読める(serde 既定)。危険な
  不可逆操作は無い。forge 側の永続は増えない(self-review は forge を触らない。escalate 時の draft は
  ADR 0021 の既存経路)。
...
1. **台帳の永続と resume**:台帳を積んだ checkpoint を serialize→deserialize しても status・
   ...
   加えて **persist 時に open な ledger エントリが `self_review_pending` に mirror されている**
   (rollback 安全弁)ことを確認。
...
5. **cap + 軽微残 → 最終 fix → publish**(finding #1 込み):ping-pong/decision 異議が無く cap 到達
   ...
   - 最終 fix 済み checkpoint(**converged=false & final_fix_unreviewed=true**)で新バイナリが
     resume すると、冒頭短絡(converged OR final_fix)で再レビューせず Continue(footer 判定も生きる)。
   - **rollback 安全弁**:最終 fix 済み checkpoint は `converged=false` なので、旧バイナリ挙動
     (rounds>=max_rounds で `escalate_unconverged`)を模したパスで resume すると escalate 側に
     倒れることを確認(未再レビュー diff が clean 公開されない)。同様に、open ledger を mirror した
     `self_review_pending` だけを読むパスで未解決 finding が拾えることを確認。
