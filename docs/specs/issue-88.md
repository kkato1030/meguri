...
- `mode = "auto"` かつ `confidence >= confidence_threshold`(既定 0.7)かつ推薦種別が `apply` に
  ...
  `meguri:plan`、`needs-human` → `meguri:needs-human`。
- 昇格のたびに **理由コメント**を1件残す(confidence / 複雑度 / 根拠 / 差し戻し手順)。
- 閾値未満・`apply` 外・`skip` / `hold` は据え置き(per-issue の書き込みなし。中央レポートには載る)。
...
- `src/config.rs`
  - `TriageConfig` に `confidence_threshold: f64`(既定 0.7)と `apply: Vec<TriageAction>`
    (既定 `["ready"]`)を追加。`apply` は `ready|plan|needs-human` を認識する typed enum の集合が望ましい
    (未知値は parse error にして誤設定を早期に弾く)。両方 `#[serde(default = ...)]`。
...
- `src/engine/triage.rs`
  ...
  - 新昇格パス `promote_one`(v1 の `propose_one` を土台に):`confidence_threshold` と `apply` ゲート、
    `real_label()`(既存)へのマッピング、既存 `meguri:triage-*` 提案ラベルの除去、本ラベル付与、
    理由コメント(昇格マーカー付き)、`triage.promoted` emit。既存の「fresh 再読で engaged なら無操作」
    「ハッシュ一致なら無操作」ガードを踏襲。
...
2. **`needs-human` は `apply` で制御。** `apply` は `ready|plan|needs-human` を認識し、既定は
   `needs-human` を含まない。昇格すると `meguri:needs-human` へ写す(着手はしない、人間へ球を渡すだけ)。
   issue の「needs-human 推薦は meguri:needs-human へ」はこのマッピング定義であり、常時昇格ではない、
   という解釈を採る。
...
- 単体(`src/engine/triage.rs` の `#[cfg(test)]`):
  ...
  - `apply` 外の種別(既定で `plan`/`needs-human`)は昇格しない。
  - `hold` / `skip` は本ラベルを持たず無操作。
