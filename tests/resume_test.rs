//! Recovery via the agent's native session: the turn contract reports
//! `agent_session_id`, a dead pane is respawned with `--resume <id>`, and a
//! rejected resume falls back to the plain full-prompt spawn.
...
async fn recovery_respawns_with_resume_when_session_is_on_record() {
    ...
    // an earlier turn recorded the agent's native session.
    env.deps
        .store
        .update_run_agent_session(&run.id, Some("sess-123"))
        .unwrap();
...
async fn rejected_resume_falls_back_to_full_prompt_spawn() {
    ...
    env.deps
        .store
        .update_run_agent_session(&run.id, Some("sess-expired"))
        .unwrap();
    ...
    // The rejected id is forgotten so the next recovery doesn't retry it.
    let record = env.deps.store.get_run(&run.id).unwrap().unwrap();
...
async fn resumed_pane_dying_without_result_forgets_the_session() {
    ...
    env.deps
        .store
        .update_run_agent_session(&run.id, Some("sess-stale"))
        .unwrap();
